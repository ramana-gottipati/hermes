# -*- coding: utf-8 -*-
"""Hold-band robustness sweep for K30-HOLD / A2-HOLD: is the C gain stable across band values, or a lucky pick?
Reuses the sealed union engine + the exact Zerodha gauntlet; band in {0=baseline,40,45,50,55,60}. CRASH off."""
import sys
...
print("\n" + "="*84)
print("HOLD-BAND ROBUSTNESS SWEEP — net CAGR vs baseline, SAME Zerodha gauntlet, 2005-2026")
print("="*84)
for k in ("K30", "A2"):
    seal = GATE[k][0]
    print("\n==== %s  (sealed flat %.1f; topn=%d) ====" % (k, seal, BOOKS[k]["topn"]))
    print("  band     flat%  NET%   1cr->   maxDD%  turn/yr   dNET")
    base = None
    for hb in (0, 40, 45, 50, 55, 60):
        r = measure(BOOKS[k], hb)
        if hb == 0:
            base = r; d = "  [seal %s]" % ("OK" if abs(r["flat"]-seal) < 0.7 else "FAIL")
        else:
            d = "  %+.1f" % (r["gauntlet"]-base["gauntlet"])
        print("  %-6s  %5.1f  %5.1f  %6.2f   %5.0f   %6.0f%s"
              % (("base" if hb == 0 else "<=%d" % hb), r["flat"], r["gauntlet"], r["cr"], r["mdd"], r["turn_yr"], d))
print("\ndone.")
'''
exec(compile(head + "\n" + DRIVER, "union_sweep", "exec"), {})
