"""UNION LADDER VALIDATION — runner for the SEALED protocol
docs/prereg/union-ladder-validation-prereg.md (sha256
37c28824e969735df0b55e879f9dc8e1d73dee924625188f73a5756b47832b56, sealed by the coordination
session BEFORE this module existed; Ramana 2026-07-16: "run the validation prereg before
graduation"). Every decision rule below is FROZEN in that file; this module only computes.

C1  paired significance of the ladder increments (U->B14->C40->A2->K30 + C40->K30 + U->K30):
    joint stationary block bootstrap (Politis-Romano, mean block 4 quarters, 10,000 draws, seed 42
    - the 15i machinery, sector_rotation_significance.py, reused verbatim in method), 95% CI of the
    annualized CAGR gap + p(gap<=0); Newey-West lag-4 paired t; JK/Memmel per-period return/vol z
    as cross-check; corr(rA,rB) reported (why paired is required).
    FROZEN RULE on C40->K30: CI includes 0 -> the graduation candidate defaults to the simpler book.
C2a rolling 3y windows x 5 books (stability read, labelled as such).
C2b the real interim OOS: generational re-search on TRAIN (rb <= 2018-12-31) only - each historical
    generation's change re-adjudicated by its own gate computed on TRAIN sub-windows (2006-11,
    2012-17, 2015-18; era floor P recomputed from TRAIN's last 12 months) - the surviving book is
    frozen, then measured ONCE on 2019+. survival reported vs BOTH constructions (as-sealed K30's
    2018-26 alpha +18.2, and the winner's own TRAIN alpha).
C3  deflated return/vol (Bailey-LdP, attribution.py:314 formula at quarterly annualization) with
    N_trials itemized IN THIS FILE from ledger 16U-16AH; forward-expectation band =
    bench_CAGR + phi * alpha published per book.
C4  let-winners-run contribution by window (K30 minus its EW twin).
C5  dead-name haircut stress on A2/K30 (-70%/-90% vs the sealed -50%) + dead-event counts.

REPRODUCTION GATE (mandatory, frozen): U 17.5%/26.04x - B14 18.1%/28.84x - C40 21.0%/47.29x -
A2 25.5%/99.03x - K30 26.4%/115.69x, all to the digit, else STOP.
...
print("=" * 118)
print("UNION LADDER VALIDATION — sealed protocol 37c28824... . Reproduction gate first; frozen rules per section.")
print("=" * 118)

RUNS, RETS, MEDADV = {}, {}, {}
rb_full = list(rebal_all)
for k, cfg in BOOKS.items():
    o = run5(**cfg)
    RUNS[k] = o
    navs = o["navs"]
    RETS[k] = [navs[i]/navs[i-1]-1 for i in range(1, len(navs))]
    s = stat(navs, o["bnavs"])
    madv = sorted(o.get("medadv") or [0])[len(o.get("medadv") or [0])//2]/1e7 if o.get("medadv") else None
    g_c, g_m = GATE[k]
    ok = abs(s["cagr"]*100 - g_c) < 0.05 and abs(s["mult"] - g_m) < 0.05
    print("  repro %-4s CAGR %5.1f%% (gate %.1f)  Rs1Cr->%6.2fx (gate %.2f)  %s"
          % (k, s["cagr"]*100, g_c, s["mult"], g_m, "OK" if ok else "FAIL"), flush=True)
    if not ok:
        print("REPRODUCTION GATE FAILED — STOP (per the sealed protocol)"); sys.exit(1)
n0 = len(RETS["U"])
assert all(len(RETS[k]) == n0 for k in RETS), "common rb grid violated"
print("  common grid: %d quarters" % n0)

# ================= C1 =================
print("")
print("### C1 — paired significance of the ladder increments (block bootstrap 10k, mean block 4, seed 42)")
PAIRS = [("U","B14"), ("B14","C40"), ("C40","A2"), ("A2","K30"), ("C40","K30"), ("U","K30")]
c1 = {}
for A, B in PAIRS:
    a, b = RETS[A], RETS[B]
    obs, gaps = paired_boot_cagr(a, b)
    lo, hi = qtile(gaps, 0.025), qtile(gaps, 0.975)
    p_le0 = sum(1 for g in gaps if g <= 0)/len(gaps)
    t = nw_t([b[i]-a[i] for i in range(len(a))])
    z = jk_z(b, a)
    c1[(A,B)] = (obs, lo, hi, p_le0)
    print("  %-9s gap %+6.2fpp  95%%CI [%+6.2f, %+6.2f]  p(<=0) %.3f  NW-t %+5.2f  JK-z %+5.2f  corr %.3f"
          % (f"{A}->{B}", obs*100, lo*100, hi*100, p_le0, t, z, corr2(a, b)), flush=True)
obs, lo, hi, p = c1[("C40","K30")]
verdict = "CI EXCLUDES 0 and p<=0.05 -> increment REAL" if (lo > 0 and p <= 0.05) else \
          "CI INCLUDES 0 -> per D139 the graduation candidate DEFAULTS TO THE SIMPLER BOOK (C40/A2)"
print("  FROZEN RULE on C40->K30: %s" % verdict)

# ================= C2a =================
print("")
print("### C2a — rolling 3y windows (STABILITY read, not OOS)")
W3 = [("2006-01-01","2008-12-31"), ("2009-01-01","2011-12-31"), ("2012-01-01","2014-12-31"),
...
for k, cfg in BOOKS.items():
    ...
    pos = 0
    for stt, en in W3:
        o = run5(start=stt, end=en, **cfg)
        if len(o["navs"]) < 4:
            cells.append("   -  "); continue
        s = stat(o["navs"], o["bnavs"])
        pos += 1 if s["alpha"] > 0 else 0
        cells.append("%+5.1f" % (s["alpha"]*100))
    print("  %-4s alpha by 3y window: %s   alpha>0 in %d/7" % (k, " ".join(cells), pos), flush=True)

# ================= C2b =================
print("")
print("### C2b — TRAIN(<=2018) generational re-search, TEST(2019+) measured once")
_bym2 = _bym  # from head
mtr = [ym for ym in sorted(_bym2) if ym <= "2018-12"]
fr = [sum(1 for v in _bym2[ym] if v >= ADV_BAR)/len(_bym2[ym]) for ym in mtr[-12:]]
P_T = sum(fr)/len(fr)
THR_T = {}
for ym in sorted(_bym2):
    vals = sorted(_bym2[ym])
    kk = min(int(len(vals)*(1.0-P_T)), len(vals)-1)
    THR_T[ym] = max(1e7, vals[kk])
FLOORS["pf1t"] = lambda ym: THR_T.get(ym, ADV_BAR)
print("  P_train (2018's last-12-months eligible fraction) = %.3f" % P_T, flush=True)
print("  [precompute] pf1t qualifiers...", file=sys.stderr, flush=True)
QUAL["pf1t"] = {d: qualify(d, "pf1t") for d in rebal_all}

TR_WINS = ((None, "2018-12-31", "full-train"), ("2006-01-01", "2011-12-31", "06-11"),
           ("2012-01-01", "2017-12-31", "12-17"), ("2015-01-01", "2018-12-31", "15-18"))
def train_rows(cfg):
    out = {}
    for stt, en, tag in TR_WINS:
        o = run5(start=stt, end=en, **cfg)
        out[tag] = stat(o["navs"], o["bnavs"]) if len(o["navs"]) >= 4 else None
    return out

def gate_beats(cand, inc, use_bd=False):
    c, i = cand, inc
    try:
        ok = (c["full-train"]["cagr"] > i["full-train"]["cagr"]
              and c["full-train"]["alpha"] > i["full-train"]["alpha"]
              and c["12-17"]["alpha"] >= 0.02
              and all(c[w]["alpha"] >= i[w]["alpha"] - 0.015 for w in ("06-11", "12-17", "15-18")))
        if use_bd:
            ok = ok and c["full-train"]["beta"] <= 0.90 and c["full-train"]["dd"] >= -0.32
        return ok
    except (KeyError, TypeError):
        return False

GENS = [
    ("g1 beta-cap", dict(fmode="base", topn=60, rf_cash=False, hook=hook_b14), False),
    ("g2 rank+top40", dict(fmode="base", topn=40, rf_cash=False), False),
    ("g3 era-floor(P_train)+rf", dict(fmode="pf1t", topn=40, rf_cash=True), False),
    ("g4 top30+drift", None, True),   # config derived from g3 winner below
]
incumbent_cfg = dict(BOOKS["U"]); incumbent_rows = train_rows(incumbent_cfg); path = ["U"]
for name, cfg, use_bd in GENS:
    if cfg is None:
        cfg = dict(incumbent_cfg); cfg.update(topn=30, weights="drift")
    rows = train_rows(cfg)
    win = gate_beats(rows, incumbent_rows, use_bd)
    print("  %-26s on TRAIN: %s" % (name, "ADOPTED" if win else "REJECTED (gate not met on TRAIN)"), flush=True)
    if win:
        incumbent_cfg, incumbent_rows = cfg, rows
        path.append(name)
print("  TRAIN-derived book path: %s" % " -> ".join(path))
o_test = run5(start="2019-01-01", **incumbent_cfg)
s_test = stat(o_test["navs"], o_test["bnavs"])
print("  TRAIN-derived book on TEST 2019+ : CAGR %5.1f%%  alpha %+5.1f%%  beta %4.2f"
      % (s_test["cagr"]*100, s_test["alpha"]*100, s_test["beta"]), flush=True)
tr_alpha = incumbent_rows["full-train"]["alpha"]
print("  survival vs as-sealed K30 2018-26 alpha (+18.2): %.2f | vs own TRAIN alpha (%+.1f): %.2f"
      % (s_test["alpha"]/0.182, tr_alpha*100, s_test["alpha"]/tr_alpha if tr_alpha else float("nan")))
print("  as-sealed books on 2019+ (for the same read):")
for k, cfg in BOOKS.items():
    o = run5(start="2019-01-01", **cfg)
    s = stat(o["navs"], o["bnavs"])
    print("    %-4s CAGR %5.1f%%  alpha %+5.1f%%  beta %4.2f" % (k, s["cagr"]*100, s["alpha"]*100, s["beta"]), flush=True)

# ================= C3 =================
print("")
print("### C3 — deflated return/vol (Bailey-LdP, quarterly) + forward-expectation bands")
TRIALS = [("16U dim6 selectors", 8), ("16V blend+sizes", 7), ("16W throttles", 3), ("16X ivol", 1),
          ("16Y/Z lab1 variants", 13), ("16Y lab2 diagnostics+quality", 9), ("16AA ML models", 2),
          ("16AB/AC lab3 variants", 13), ("16AC blend", 1), ("16AE lab4 A1/A2/B1+composite", 4),
          ("16AF lab5 variants+composite", 6), ("16AG ML2 models", 2)]
N_TRIALS = sum(n for _t, n in TRIALS)
print("  N_trials = %d  (itemized: %s)" % (N_TRIALS, "; ".join(f"{t}={n}" for t, n in TRIALS)))
bench_full = bench_cagr_series(iclose[SLEEVE], rb_full)
for k in BOOKS:
    s = stat(RUNS[k]["navs"], RUNS[k]["bnavs"])
    dsr, phi, rv_ann, rv0_ann = deflated_rv(RETS[k], N_TRIALS)
    band = (bench_full or 0) + max(phi, 0.0)*s["alpha"]
    print("  %-4s in-sample CAGR %5.1f%% (alpha %+5.1f)  ann rv %4.2f vs E[max|null] %4.2f  DSR %.3f  phi %+.2f"
          "  -> deflated forward-CAGR expectation ~ %4.1f%%"
          % (k, s["cagr"]*100, s["alpha"]*100, rv_ann, rv0_ann, dsr, phi, band*100), flush=True)

# ================= C4 =================
print("")
print("### C4 — let-winners-run contribution by window (K30 minus its EW twin)")
for stt, en, tag in ((None, None, "FULL"), ("2006-01-01","2011-12-31","06-11"),
                     ("2012-01-01","2017-12-31","12-17"), ("2018-01-01","2026-12-31","18-26")):
    o_d = run5(start=stt, end=en, fmode="pf1", topn=30, rf_cash=True, weights="drift")
    o_e = run5(start=stt, end=en, fmode="pf1", topn=30, rf_cash=True, weights="ew")
    sd_, se_ = stat(o_d["navs"], o_d["bnavs"]), stat(o_e["navs"], o_e["bnavs"])
    print("  %-6s drift %5.1f%% vs EW %5.1f%%  -> contribution %+4.1fpp"
          % (tag, sd_["cagr"]*100, se_["cagr"]*100, (sd_["cagr"]-se_["cagr"])*100), flush=True)

# ================= C5 =================
print("")
print("### C5 — dead-name haircut stress (sealed -50% vs -70%/-90%)")
for k in ("A2", "K30"):
    base_s = stat(RUNS[k]["navs"], RUNS[k]["bnavs"])
    line = ["  %-4s sealed %5.1f%% (dead events %d)" % (k, base_s["cagr"]*100, RUNS[k]["ndead"])]
    for dv in (-0.70, -0.90):
        o = run5(dead=dv, **BOOKS[k])
        s = stat(o["navs"], o["bnavs"])
        line.append("@%d%% -> %5.1f%% (%+.1fpp)" % (int(dv*100), s["cagr"]*100, (s["cagr"]-base_s["cagr"])*100))
