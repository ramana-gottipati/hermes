...
print("=" * 118)
print("ARE 6b AND THE RSI STACK COMPLEMENTARY? overlap + return-correlation diagnostics")
print("=" * 118)

# --- 1. OVERLAP: on each rebalance, how many names does each pick, and how many SHARED? ---
SIGNAL = "union"
n_rsi = n_6b = n_both = n_dates = 0
for k in range(len(rebal_all) - 1):
    d = rebal_all[k]; i = ci[d]
    tags = {}
    candidates(d, consist=0.70, tag_membership=tags)
    if not tags: continue
    n_dates += 1
    r = sum(1 for v in tags.values() if v in ("rsi", "both"))
    b = sum(1 for v in tags.values() if v in ("6b", "both"))
    bo = sum(1 for v in tags.values() if v == "both")
    n_rsi += r; n_6b += b; n_both += bo
print("  avg names/quarter:  RSI-eligible %.1f | 6b-eligible %.1f | in BOTH %.1f"
      % (n_rsi / n_dates, n_6b / n_dates, n_both / n_dates))
overlap = n_both / max(min(n_rsi, n_6b), 1)
print("  OVERLAP: %.1f%% of the smaller set is shared. Low overlap => the two pick DIFFERENT stocks."
      % (overlap * 100))

# --- 2. RETURN CORRELATION: quarter-by-quarter returns of the two standalone books ---
def qseries(sig):
    global SIGNAL
    SIGNAL = sig
    navs, bnavs, inv = run(mode="sleeve200", consist=(0.70 if sig == "rsi" else None))
    return [navs[j] / navs[j - 1] - 1 for j in range(1, len(navs))]
r_rsi = qseries("rsi"); r_6b = qseries("6b")
n = min(len(r_rsi), len(r_6b))
mr, mb = sum(r_rsi[:n]) / n, sum(r_6b[:n]) / n
import math
cov = sum((r_rsi[j] - mr) * (r_6b[j] - mb) for j in range(n)) / (n - 1)
sr = math.sqrt(sum((r_rsi[j] - mr) ** 2 for j in range(n)) / (n - 1))
sb = math.sqrt(sum((r_6b[j] - mb) ** 2 for j in range(n)) / (n - 1))
corr = cov / (sr * sb) if sr and sb else 0.0
print("  RETURN CORRELATION between the two books (quarterly): %.2f" % corr)
print("  (low corr => genuinely complementary; high corr => redundant, blend just dilutes)")

# --- 3. THE BLEND BOOKS ---
print("")
print("--- the blend, sleeve200 ---")
d0, d1 = rebal_all[0], rebal_all[-1]; yy = (len(rebal_all) - 1) / 4.0
for nm in (BENCH, SLEEVE):
    a, b = iclose[nm].get(d0), iclose[nm].get(d1)
    if a and b: print("  %-46s CAGR %5.1f%%  Rs1Cr->%6.2fx" % (nm + " buy&hold", ((b / a) ** (1 / yy) - 1) * 100, b / a))
SIGNAL = "rsi"; go("RSI+consist70 alone", mode="sleeve200", consist=0.70)
SIGNAL = "6b"; go("6b alone", mode="sleeve200", consist=None)
SIGNAL = "union"; go("UNION (either signal fires)", mode="sleeve200", consist=0.70)
SIGNAL = "inter"; go("INTERSECTION (both must fire)", mode="sleeve200", consist=0.70)

print("")
print("--- book size on the UNION ---")
SIGNAL = "union"
for nn in (40, 60, 80):
    go("union, top%d" % nn, mode="sleeve200", consist=0.70, topn=nn)

print("")
print("=" * 118)
print("WALK-FORWARD - the UNION (do the two failures cancel?)")
print("=" * 118)
SIGNAL = "union"
for tag, stt, en in (("2006-2011", "2006-01-01", "2011-12-31"), ("2012-2017", "2012-01-01", "2017-12-31"), ("2018-2026", "2018-01-01", "2026-12-31")):
    navs, bnavs, inv = run(mode="sleeve200", consist=0.70, start=stt, end=en)
...
print("")
print("  RSI stack failed 2012-2017 (alpha -3.5%); 6b failed 2018-2026 (alpha -4.1%).")
print("  If the union is POSITIVE in BOTH, the failures are complementary and the blend is real.")
