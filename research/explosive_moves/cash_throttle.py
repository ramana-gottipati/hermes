...
def market_stretch(i):
    """how far Nifty 500 sits above its own 200DMA, as a fraction. >0 = extended."""
    if b200[i] is None: return 0.0
    return iclose[BENCH][cal[i]]/b200[i] - 1.0

def throttle_factor(i, mode):
    """multiplier in [floor,1] on the invested fraction. mode names the 'extended' measure."""
    if mode is None or mode == "off":
        return 1.0
    st = market_stretch(i)
    if mode == "linear":
        # full size at/below the 200DMA; scale down linearly as it stretches, floor 0.4 at +20%
        if st <= 0: return 1.0
        return max(0.4, 1.0 - st/0.20*0.6)
    if mode == "step":
        # >10% above 200DMA -> half size
        return 0.5 if st > 0.10 else 1.0
    if mode == "hard":
        # >15% above 200DMA -> quarter size (aggressive)
        if st > 0.15: return 0.25
        if st > 0.07: return 0.6
        return 1.0
    return 1.0

def run(mode="norm", topn=40, trail=0.20, slip=0.01, consist=0.70, max_inv=1.0, start=None, end=None, throttle=None):
    ...
    for k in range(len(rb)-1):
        ...
        tf = throttle_factor(i, throttle)
        if tf < 1.0 and w:
            w = {s: x*tf for s, x in w.items()}      # THROTTLE: shrink invested fraction when extended
...
print("=" * 118)
print("THROTTLE THE UNION when the market is EXTENDED (fix the 2012-17 beta-1.56 failure)")
print("=" * 118)
...
SIGNAL = "union"
print("")
print("--- full period, union top60, each throttle ---")
for th in (None, "linear", "step", "hard"):
    go("throttle=%-7s" % str(th), mode="sleeve200", consist=0.70, topn=60, throttle=th)

print("")
print("=" * 118)
print("WALK-FORWARD with throttle — does it rescue 2012-2017 without wrecking the others?")
print("=" * 118)
for th in ("off", "linear", "step", "hard"):
    print("--- throttle = %s ---" % th)
    tval = None if th == "off" else th
    for tag, stt, en in (("2006-2011", "2006-01-01", "2011-12-31"), ("2012-2017", "2012-01-01", "2017-12-31"), ("2018-2026", "2018-01-01", "2026-12-31")):
        navs, bnavs, inv = run(mode="sleeve200", consist=0.70, topn=60, start=stt, en=en) if False else run(mode="sleeve200", consist=0.70, topn=60, start=stt, end=en, throttle=tval)
