"""UNION LAB 4b — the two levers left standing: the ERA-RELATIVE ADV FLOOR and the RF-EARNING
...
without a forward result", union-c40ra-prereg.md). A winner here is recorded as a DEFERRED LEAD in
the ledger — registration waits for the 2026-10-03 forward verdict unless Ramana explicitly
reopens the family. Nothing here touches any seal.

THE DIAGNOSIS THAT MOTIVATES LEVER A (measured, 2026-07-16): the sealed ABSOLUTE Rs5cr ADV floor
admits 389 names in 2006 and 344-424 in 2011-2013 vs ~1,500-1,600 in 2024-26 — a ~4x tighter
universe exactly in the windows where the C40RA book starves (2006-11 avg n=29 of 40, inv 73%).
The floor is nominal; the market's traded value grew ~10x; the filter therefore drifts ever-looser
in real terms. LEVER A makes it era-relative: a PIT monthly PERCENTILE floor.

  CALIBRATION RULE (declared BEFORE any run, no sweeping): P = the mean, over the LAST 12 complete
  months of data, of the fraction of ADV-reporting symbols with ADV >= Rs5cr (the sealed floor).
  The monthly threshold THR[ym] = that month's (1-P)-quantile of the ADV cross-section. So the
  MODERN universe is ~unchanged by construction; only the early era widens.
  A1 = THR floor · A2 = max(Rs1cr, THR) (dust clamp — the secondary, declared variant).

LEVER B1 (rf-earning cash): every union-family book to date pays 0% on idle capital in the bear
state. India's overnight rate is 6-8% for most of the window. Fix per the ESTATE'S OWN documented
convention (attribution.py rf_monthly): "Nifty 1D Rate Index" where it exists (2016-06-23+, in
index_rows), flat RF_PROXY_ANN = 6.5%/yr before (attribution.py:76). Applied ONLY to the
bear-state idle (Nifty 500 < 200DMA); the healthy-state sleeve stays Next-50 (V17 law untouched).

LEVER B2 (G-sec bear sleeve): DATA-BLOCKED today — index_rows holds only dead 2014-15 GSEC
fragments and no long G-sec/gold history; no history-fetch tool exists in the repo. Recorded, and
queued onto the TRI/rf ingestion lane (one primary-source feed lane brings TRI + G-sec + rate
histories together). NOT attempted by scraping ad-hoc (Guardrail #8 discipline: do it properly).
...
def run_lagged(fmode="pf", hook=sel_c40ra, topn=40, trail=0.20, slip=0.01, rf_cash=False,
               tr=False, start=None, end=None):
    """D5-F1 honest-execution variant: signals at close d, trades at close d+1."""
    rb = [d for d in rebal_all if (start is None or d >= start) and (end is None or d <= end)]
    nav = bnav = 1.0
    ...
    for k in range(len(rb)-1):
        d, dn = rb[k], rb[k+1]
        i = ci[d]; i1 = ci[dn]
        e0 = cal[min(i+1, N-1)]; e1 = cal[min(i1+1, N-1)]
        sel = hook(QUAL[fmode][d], d, i, topn)[:topn]
        w = {s: 1.0/topn for s in sel}
        nsel.append(len(sel))
        turn = sum(abs(w.get(s, 0)-held.get(s, 0)) for s in set(w)|set(held))
        ...
        for s in w:
            if s not in held:
                p = pxn(s, e0); ent[s] = p; pk[s] = p or 0
        held = w
        inv = sum(held.values()); invfrac.append(inv)
        r = 0.0
        for s, x in list(held.items()):
            a, b = pxn(s, e0), pxn(s, e1)
            if a and b:
                hit = False; hit_j = None
                if trail and ent.get(s):
                    for j in range(min(i+2, N-1), min(i1+2, N-1)+1):
                        q_ = sclose[s].get(cal[j])
                        if not q_: continue
                        if q_ > pk.get(s, 0): pk[s] = q_
                        if q_ <= pk[s]*(1-trail):
                            r += x*((pk[s]*(1-trail)*(1-slip))/a-1.0); hit = True; hit_j = j; break
                if tr and DIVS.get(s):
                    stop_j = hit_j if hit_j is not None else min(i1+1, N-1)
                    ra = rawpxn(s, e0)
                    if ra:
                        for ex, amt in DIVS[s]:
                            if e0 < ex <= cal[stop_j]:
                ...
                if hit:
                    ...
                    r += x*(b/a-1.0)
            elif a and isdead(s, e1):
                r += x*DEAD_VAL; del held[s]
        idle = max(0.0, 1.0 - inv)
        if idle > 0:
            healthy = b200[i] is not None and iclose[BENCH][cal[i]] >= b200[i]
            if healthy:
                a2, b2 = iclose[SLEEVE].get(e0) or iclose[SLEEVE].get(d), iclose[SLEEVE].get(e1) or iclose[SLEEVE].get(dn)
                if a2 and b2: r += idle*(b2/a2-1.0)
            elif rf_cash:
                r += idle*rf_q(d, dn)
        nav *= (1+r); bnav *= iclose[BENCH][dn]/iclose[BENCH][d]
        navs.append(nav); bnavs.append(bnav)
    return dict(navs=navs, bnavs=bnavs, inv=invfrac, nsel=nsel, medadv=[], ndiv=ndiv)

def battery2(tag, fn=run, **kw):
    print("")
    print("### %s" % tag, flush=True)
    for stt, en, wtag in WIN:
        o = fn(start=stt, end=en, **kw)
        if len(o["navs"]) < 8:
            print("  %-14s too short" % wtag); continue
        s = stat(o["navs"], o["bnavs"])
        extra = ("  div %d" % o["ndiv"]) if kw.get("tr") else ""
        print("  %-14s CAGR %5.1f%%  MaxDD %6.1f%%  Rs1Cr->%6.2fx  beta %5.2f  alpha %+5.1f%%%s"
              % (wtag, s['cagr']*100, s['dd']*100, s['mult'], s['beta'], s['alpha']*100, extra), flush=True)

print("=" * 118)
print("UNION LAB 4b — STRESS on the era-floor composites (A1=pf, A2=pf1; both + rf-cash). Small-ADV tail => harsher slips.")
print("=" * 118)
battery2("A1-composite control (repro 25.6)", fmode="pf", rf_cash=True)
battery2("A1-composite @2% slip", fmode="pf", rf_cash=True, slip=0.02)
battery2("A1-composite @3% slip", fmode="pf", rf_cash=True, slip=0.03)
battery2("A1-composite LAGGED", fn=run_lagged, fmode="pf", rf_cash=True)
battery2("A1-composite LAGGED @2% TR (worst honest)", fn=run_lagged, fmode="pf", rf_cash=True, slip=0.02, tr=True)
battery2("A2-composite (pf1 + rf)", fmode="pf1", rf_cash=True)
battery2("A2-composite @2% slip", fmode="pf1", rf_cash=True, slip=0.02)
battery2("A2-composite @3% slip", fmode="pf1", rf_cash=True, slip=0.03)
battery2("A2-composite LAGGED", fn=run_lagged, fmode="pf1", rf_cash=True)
battery2("A2-composite LAGGED @2% TR (worst honest)", fn=run_lagged, fmode="pf1", rf_cash=True, slip=0.02, tr=True)
print("")
