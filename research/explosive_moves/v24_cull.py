...
STOP = float(sys.argv[2]) if len(sys.argv) > 2 else 0.0    # hard stop from ENTRY close
TRAIL = float(sys.argv[3]) if len(sys.argv) > 3 else 0.0   # trailing stop from PEAK close


def _cull_ret(s, d, dn, ent, pk):
    """Walk DAILY closes d->dn. If the sector breaks its stop, exit AT the stop level and
    return (realised_return, True). Else return (month_return, False) and update the peak.
    Daily-close checked (not intraday low) -- indices have no low in index_rows. Disclosed."""
    i0, i1 = idx.get(d), idx.get(dn)
    c0 = close[s].get(d)
    if i0 is None or i1 is None or not c0:
        return (ret(s, d, dn) or 0.0), False
    for j in range(i0 + 1, i1 + 1):
        px = close[s].get(cal[j])
        if not px:
            continue
        if px > pk[s]:
            pk[s] = px
        if STOP and ent.get(s) and px <= ent[s] * (1 - STOP):
            return (ent[s] * (1 - STOP)) / c0 - 1.0, True
        if TRAIL and pk.get(s) and px <= pk[s] * (1 - TRAIL):
            return (pk[s] * (1 - TRAIL)) / c0 - 1.0, True
    return (ret(s, d, dn) or 0.0), False
...
def simulate_v24(record_book=False):
    ...
    for k in range(len(rebal)-1):
        ...
        if is_q:
            ...
            for s in w:
                if s not in prev:                       # fresh entry -> reset stop refs
                    ent[s] = close[s].get(d)
                    pk[s] = close[s].get(d) or 0.0
            if record_book:
                inv = sum(w.values())
                ...
                book.append(dict(date=d, holdings=[(s, round(x,4)) for s,x in sorted(w.items(), key=lambda kv:-kv[1])],
                    ...
                    entered=sorted(set(w)-set(prev)), exited=sorted(set(prev)-set(w)),
                    moved=[], turnover=round(turn,4)))
        else:
            w = dict(prev)
        ...
        for s, x in list(w.items()):
            if STOP or TRAIL:
                r_s, hit = _cull_ret(s, d, dn, ent, pk)
            else:
                r_s, hit = (ret(s, d, dn) or 0.0), False
            rp += x * r_s
            if hit:
                culled.append(s); nculled[0] += 1
        inv = sum(w.values())
        if not w:
            rp = sleeve_ret(NEXT50, d, dn) if on_index else 0.0
        elif inv < 1.0 and on_index:
            rp += (1.0-inv)*sleeve_ret(NEXT50, d, dn)
        for s in culled:                                # culled -> out until next rebalance
            w.pop(s, None); ent.pop(s, None); pk.pop(s, None)
        if is_q:
            rp -= t*COST
        rows.append((dn, rp, rb)); prev = w
    print("CULLED %d" % nculled[0], file=sys.stderr)
    if record_book:
        return rows, book
    return rows
...
out["book"] = v24_book
import sys as _s
print("STATS " + json.dumps(stats(v24_rows)), file=_s.stderr)
print("BENCH " + json.dumps(stats(n500_rets, is_pairs=False)), file=_s.stderr)
