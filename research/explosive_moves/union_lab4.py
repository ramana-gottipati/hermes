...
print("=" * 118)
print("UNION LAB 4 — era-relative ADV floor + rf-earning cash, on the C40RA base. Bars + compose rule in docstring.")
print("=" * 118)

R = {}
R["b14"] = battery("b14 control (repro 18.1)", fmode="base", hook=sel_b14, topn=60)
R["c40"] = battery("c40ra control (repro 21.0)", fmode="base", hook=sel_c40ra, topn=40)
R["a1"] = battery("A1 pct-floor (era-relative)", fmode="pf", hook=sel_c40ra, topn=40)
R["a2"] = battery("A2 pct-floor clamped >=Rs1cr", fmode="pf1", hook=sel_c40ra, topn=40)
R["b1"] = battery("B1 rf-earning cash (bear-state idle)", fmode="base", hook=sel_c40ra, topn=40, rf_cash=True)
...
def meas_pass(rows):
    ...
    try:
        return (rows["FULL 2006-2026"]["cagr"] > c["FULL 2006-2026"]["cagr"]
                and rows["FULL 2006-2026"]["alpha"] >= c["FULL 2006-2026"]["alpha"] - 0.001
                and all(rows[w]["cagr"] >= c[w]["cagr"] - 0.003
                        for w in ("2006-2011", "2012-2017", "2018-2026")))
    except KeyError:
        return False

a_win = "pf" if cand_pass(R["a1"]) else ("pf1" if cand_pass(R["a2"]) else None)
b_win = meas_pass(R["b1"])
print("")
print("AXIS VERDICTS: A(candidate bar)=%s  B1(measurement bar)=%s" % (a_win, b_win))
if a_win and b_win:
    battery("COMPOSITE (A-winner + rf-cash)", fmode=a_win, hook=sel_c40ra, topn=40, rf_cash=True)
    battery("COMPOSITE TR", fmode=a_win, hook=sel_c40ra, topn=40, rf_cash=True, tr=True)
elif a_win:
    battery("A-winner TR", fmode=a_win, hook=sel_c40ra, topn=40, tr=True)
elif b_win:
    battery("B1 TR", fmode="base", hook=sel_c40ra, topn=40, rf_cash=True, tr=True)
else:
