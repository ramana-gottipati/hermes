...
def candidates(d, consist=0.70):
    ...
    for s, sec in amap.items():
        ...
        if SIGNAL == "6b":
            if not rsi_of_rs_recovery(s, sec, i): continue
        else:
            r, m = RSI[s][i], RMA[s][i]
...
print("=" * 118)
print("6b RSI-of-RS OVERSOLD RECOVERY - FULL BOOK TEST")
print("=" * 118)
d0, d1 = rebal_all[0], rebal_all[-1]
yy = (len(rebal_all) - 1) / 4.0
for nm in (BENCH, SLEEVE):
    a, b = iclose[nm].get(d0), iclose[nm].get(d1)
    if a and b:
        print("  %-46s CAGR %5.1f%%  Rs1Cr->%6.2fx" % (nm + " buy&hold", ((b / a) ** (1 / yy) - 1) * 100, b / a))

SIGNAL = "6b"
print("")
print("--- 6b ALONE (no consistency filter) ---")
go("6b, norm (100pct invested)", mode="norm", consist=None)
go("6b, cash (idle to cash)", mode="cash", consist=None)
go("6b, sleeve200 (idle to Next50>200DMA)", mode="sleeve200", consist=None)

print("")
print("--- 6b + consistency>=70pct ---")
go("6b + consist70, norm", mode="norm", consist=0.70)
go("6b + consist70, sleeve200", mode="sleeve200", consist=0.70)

print("")
print("--- book size on 6b + consist70 + sleeve200 ---")
for n in (20, 40, 60):
    go("6b + consist70, sleeve200, top%d" % n, mode="sleeve200", consist=0.70, topn=n)

print("")
print("--- head-to-head: 6b vs RSI(price) stack, sleeve200 top40 ---")
SIGNAL = "6b"
go("6b selector", mode="sleeve200", consist=0.70)
SIGNAL = "rsi"
go("RSI(price)>50MA selector", mode="sleeve200", consist=0.70)

print("")
print("=" * 118)
print("WALK-FORWARD - 6b + consist70 + sleeve200")
print("=" * 118)
SIGNAL = "6b"
for tag, st, en in (("2006-2011", "2006-01-01", "2011-12-31"), ("2012-2017", "2012-01-01", "2017-12-31"), ("2018-2026", "2018-01-01", "2026-12-31")):
    navs, bnavs, inv = run(mode="sleeve200", consist=0.70, start=st, end=en)
    if len(navs) < 8:
        print("  %s: too short" % tag); continue
    st_ = stat(navs, bnavs)
