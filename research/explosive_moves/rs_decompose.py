...
print("=" * 80)
print("WHY DOES A BOOK OF INDEX-BEATING STOCKS LOSE TO THE INDEX?")
print("=" * 80)
for dm, label in (("kill", "dead = -100% (brutal bound)"),
                  ("drop", "dead = excluded (survivorship-biased bound)"),
                  ("flat", "dead = 0% (the 15j/15k treatment)")):
    DEAD_MODE = dm
    _DEADRATE = []
    print("\n" + "#" * 80)
    print(f"### DEAD-NAME TREATMENT: {label}")
    print("#" * 80)
    run("FULL PERIOD 2005-2026", 2005, 2026)
    if _DEADRATE:
        print(f"  [avg {sum(_DEADRATE)/len(_DEADRATE)*100:.2f}% of the universe dies per quarter]")

DEAD_MODE = "kill"
print("\n" + "#" * 80)
print("### ERA BREAKDOWN (dead = -100%)")
print("#" * 80)
run("ERA 1: 2005-2010", 2005, 2010)
run("ERA 2: 2011-2016", 2011, 2016)
run("ERA 3: 2017-2026", 2017, 2026)
