"""UNION LAB 3b — the 25%-CAGR push: single-axis CAGR levers on the beta-cap-1.4 base + the
...
def run_lagged(qual, rb_dates, hook, topn=60, trail=0.20, slip=0.01, sleeve=SLEEVE,
               tr=False, start=None, end=None):
    """D5-F1 honest-execution variant: signals read at close d, but ENTRY/EXIT prices are the
    NEXT trading day's close (cal[i+1] / day after dn); stop scan starts one day later too.
    Everything else identical to run()."""
    rb = [d for d in rb_dates if (start is None or d >= start) and (end is None or d <= end)]
    nav = bnav = 1.0
    navs, bnavs, invfrac, nsel, turns = [], [], [], [], []
    ndiv = 0
    ...
    for k in range(len(rb)-1):
        ...
        nav *= (1+r); bnav *= iclose[BENCH][dn]/iclose[BENCH][d]
        navs.append(nav); bnavs.append(bnav)
    return dict(navs=navs, bnavs=bnavs, inv=invfrac, nsel=nsel, turns=turns, ppy=4.0, ndiv=ndiv)
...
def battery(tag, fn=run, **kw):
    print("")
    print("### %s" % tag, flush=True)
    for stt, en, wtag in WIN:
        o = fn(QUAL, rebal_all, start=stt, end=en, **kw)
        if len(o["navs"]) < 8:
            print("  %-14s too short" % wtag); continue
        s = stat(o["navs"], o["bnavs"], o["ppy"])
...
print("=" * 118)
print("UNION LAB 3b — composite (b14 + top40 + riskadj-rank) honesty passes: slip-2 + D5-F1 next-day execution")
print("=" * 118)
battery("composite (control repro, same-close)", hook=sel_b14_riskadj, topn=40)
battery("composite @2% stop-slip", hook=sel_b14_riskadj, topn=40, slip=0.02)
battery("composite LAGGED (D5-F1 next-day entry)", fn=run_lagged, hook=sel_b14_riskadj, topn=40)
battery("composite LAGGED TR", fn=run_lagged, hook=sel_b14_riskadj, topn=40, tr=True)
battery("composite LAGGED @2% slip TR (worst honest case)", fn=run_lagged, hook=sel_b14_riskadj, topn=40, slip=0.02, tr=True)
print("")
