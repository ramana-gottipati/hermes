...
for s, ym, lo in conn.execute("""
    ...
    FROM bhavcopy_rows WHERE series IN ('EQ','BE','BZ') AND low>0 GROUP BY 1,2"""):
    ml[s][ym] = lo
...
def simulate(start_ym, stop=None, trail=None, rs_exit=False, cost=COST, topn=TOPN, slip=0.0,
             dead_val=0.0):
    """dead_val = what a position is worth when the stock VANISHES while held (no next close,
    so there is no low to stop against -- a suspension traps you; the stop cannot save you).
    0.0 = carried flat (what 15j/15k assumed -- shown by ledger 15L to equal survivorship bias).
    -1.0 = total loss. The truth is between: buyouts pay, frauds do not."""
        ...
        if pos:
            ...
            for s in list(pos):
                ...
                if stop and lo is not None and lo <= p['entry'] * (1 - stop):
                    ...
                    r = dead_val; fired['dead'] += 1          # vanished while held: no low to
                    del pos[s]                                # stop against -- suspension traps you
...
print("\n### CORRECTED UNIVERSE (EQ+BE+BZ per ledger 15L — surveillance moves are NOT deaths).")
print("### THE QUESTION: does a CULL rule close the -1.24%/qtr pond gap vs Nifty 500?\n")

print("#" * 78)
print("### A. does the cull beat the index? (dead=-100%: the surviving vanishes are REAL now)")
print("#" * 78)
show("  NO EXIT (the pond, unculled)", "2005-01", dead_val=-1.0)
show("  HARD -15%", "2005-01", stop=0.15, dead_val=-1.0)
show("  TRAIL -20%", "2005-01", trail=0.20, dead_val=-1.0)
show("  TRAIL -15%", "2005-01", trail=0.15, dead_val=-1.0)

print("\n" + "#" * 78)
print("### B. sensitivity to what a REAL delisting is worth (only 0.47%/qtr of universe now)")
print("#" * 78)
for dv in (0.0, -0.50, -1.00):
    show(f"  HARD -15%, dead={dv*100:.0f}%", "2005-01", stop=0.15, dead_val=dv)

print("\n" + "#" * 78)
print("### C. THE KILLER: does it survive gap slippage + real stock costs?")
print("#" * 78)
for sl in (0.0, 0.01, 0.02):
    show(f"  HARD -15%, slip {sl*100:.0f}% @0.30%/side", "2005-01",
         stop=0.15, slip=sl, cost=0.0030, dead_val=-1.0)

print("\n" + "#" * 78)
print("### D. WINDOW STABILITY at the honest config (slip 1%, 0.30%/side, dead=-100%)")
print("#" * 78)
for st in ("2005-01", "2011-01", "2017-01"):
    show(f"  HARD -15% from {st[:4]}", st, stop=0.15, slip=0.01, cost=0.0030, dead_val=-1.0)
