...
Native in the fork (each patch applied by scratch generator, asserted exactly-once):
  P1 §M — the compare set lives in ?cmp= (init from URL, replaceState on change,
          sessionStorage carryover) + "Open in Compare →" hand-off (cap 12 there).
  P2 §N — drawings dense rail: ⭐ pins (seed trend/hline/fib) + MRU + grouped
          "All tools ▾" dropdown; modifiers stay outside as modes.
  P3 §N — indicator row: VWAP + Delivery % pinned, the rest in "More ▾" (row >6 with
          the injected MA chips counted).
  P4 §M.1 — the shared two-frame benchmark selector (Nifty 500 / Nifty 50).
  P5 — server-side series CSV (/dash/preview/stock/export, playbook item 8).
Seams preserved verbatim: window.__wfpc/__wfcandle · [data-ptf] · #stratBar · #cprBar ·
#maBar · __cmpAdd/__cmpRemove — the committed overlay modules bind unmodified (seam test).

Python side: the bounded data island (the dash_stock series query + CA back-adjustment +
zone lines, default 756 sessions; range=max serves full history) + the CSV route +
chart_html() (scaffold DOM + island + LWC + this snippet + CPR/MA/MEP overlay snippets).
"""
...
def _island(sym: str, max_sessions: int = DEFAULT_SESSIONS) -> dict:
    """The chart data island — the dash_stock assembly, bounded. {"series", "zones"}."""
    from src.automation import adjust
    from src.web.dashboard import _action_events, _safe_int
    with get_conn() as conn:
        rows = conn.execute(
            """SELECT b.trade_date, b.open, b.high, b.low, b.close, b.prev_close,
               ...
               ORDER BY b.trade_date DESC""" + (" LIMIT ?" if max_sessions else ""),
            ((sym, max_sessions) if max_sessions else (sym,))).fetchall()
        latest = conn.execute(
            "SELECT * FROM stock_signals WHERE symbol=? ORDER BY trade_date DESC LIMIT 1",
            (sym,)).fetchone()
        events = _action_events(conn, sym)
    series = []
    for r in reversed([dict(x) for x in rows]):
        ...
        series.append({
            "time": r["trade_date"], "open": o, "high": hi, "low": lo, "close": c,
            "prev_close": r["prev_close"], "dvpt": _safe_int(r["dvpt"]),
    ...
    adj_rows = [{"trade_date": s["time"], "close": s["close"], "prev_close": s["prev_close"]}
                for s in series]
    factors = adjust.adjustment_factors(adj_rows, events=events)
    ...
    if latest is not None:
        L = dict(latest)
        for label, col, color in (("P1M", "avg_close_p1m", "#f0b429"),
            ...
            v = L.get(col)
            if v:
                zones.append({"label": label, "price": round(v, 2), "color": color})
    return {"series": series, "zones": zones}


@router.get("/dash/preview/stock/export", include_in_schema=False)
def series_csv(sym: str = "", range: str = "") -> PlainTextResponse:
    """The chart's daily series as server-side CSV (playbook item 8; honors range=max)."""
    sym = str(sym or "").strip().upper()[:20]
    if not sym:
        return PlainTextResponse("symbol required", status_code=400)
    try:
        data = _island(sym, 0 if range == "max" else DEFAULT_SESSIONS)
    except Exception:
        return PlainTextResponse("no data", status_code=200)
    buf = _io.StringIO()
    w = _csvmod.writer(buf)
    w.writerow(["trade_date", "open", "high", "low", "close", "dvpt",
                "deliv_pct", "r1m", "traded_value", "delivered_value"])
    for s in data["series"]:
        w.writerow([s["time"], s["open"], s["high"], s["low"], s["close"], s["dvpt"],
                    s["deliv"], s["r1m"], s["tval"], s["dval"]])
    return PlainTextResponse(buf.getvalue(), media_type="text/csv",
                             headers={"Content-Disposition":
                                      "attachment; filename=%s-series.csv" % sym})


def chart_html(sym: str, rng: str = "") -> str:
    """The chart section body: scaffold DOM + data island + LWC + the forked snippet +
    the CPR/MA/MEP overlay snippets (their chips inject into the preserved anchors)."""
    try:
        data = _island(sym, 0 if rng == "max" else DEFAULT_SESSIONS)
    except Exception:
        return '<div class="pv3-dock-empty">No tape data to chart.</div>'
    if not data["series"]:
        return '<div class="pv3-dock-empty">No tape data to chart.</div>'
    try:
        from src.web.cpr_overlay import SNIPPET as _CPR
    except Exception:
        _CPR = ""
    try:
        from src.web.indicators_overlay import SNIPPET as _MA
    except Exception:
        _MA = ""
    try:
        from src.web.mep_overlay import SNIPPET as _MEP
    except Exception:
        _MEP = ""
    scaffold = (
        '<div class="fbar" id="ivBar">'
        '<button data-ptf="D" class="on">D</button><button data-ptf="W">W</button>'
        '<button data-ptf="M">M</button><button data-ptf="Q">Q</button></div>'
        '<div class="rangebar" style="display:inline-flex;gap:4px;margin-left:8px">'
    ...
    return (scaffold
            ...
            + "<script>window.__wfdata=" + _json.dumps(data) + ";</script>"
            + SNIPPET + _CPR + _MA + _MEP)
...
def _selftest() -> int:
    ...
    assert "/dash/preview/stock/export" in routes
    print("stock_chart_v3 selftest OK — seams preserved + P1..P5 present; base " + BASE_MD5[:8])
    return 0


# ── the forked snippet (generated; see module docstring) ─────────────────────────────
SNIPPET = """<script>
(function(){
  "use strict";
  ...
  function boot(){
    ...
    indBar.appendChild(regChip('vwap')); indBar.appendChild(regChip('deliv'));
    // \u00a7N (M4 inc-2): the remaining indicator chips live in a 'More \u25be' drawer (row >6 w/ MA injections)
    var indMoreBtn=E('span','cursor:pointer;font-size:11px;color:var(--ink-2);border:1px solid var(--line-2);border-radius:6px;padding:3px 8px','More \u25be');
    var indDD=E('div','display:none;position:absolute;top:100%;left:0;z-index:30;margin-top:4px;min-width:170px;background:#0e1320;border:1px solid '+C.border+';border-radius:8px;padding:6px;box-shadow:0 6px 20px rgba(0,0,0,.5);display:none');
    var indWrap=E('span','position:relative;display:inline-block'); indWrap.appendChild(indMoreBtn); indWrap.appendChild(indDD);
    indMoreBtn.onclick=function(){ indDD.style.display=(indDD.style.display==='none'||!indDD.style.display)?'block':'none'; };
    var avChip=regChip('avwap'); indDD.appendChild(avChip);
    var bbChip=regChip('bb'); bbChip.title='Bollinger Bands (20, 2\\u03c3) \\u2014 descriptive volatility envelope around a 20-period mean, not a buy/sell signal.'; indDD.appendChild(bbChip);
    var atrChip=regChip('atr'); atrChip.title='ATR bands (close \\u00b1 2\\u00d7ATR-14) \\u2014 descriptive volatility envelope, not a buy/sell signal.'; indDD.appendChild(atrChip);
    indDD.appendChild(regChip('flow'));
    indBar.appendChild(indWrap);
    ...
    function renderCmpBar(){ if(!cmpBar) return; cmpBar.innerHTML=''; cmpBar.style.position='relative';
      ...
      // shared benchmark set (\u00a7M.1): one selector, the same two frames as the RS surfaces
      var bsel=document.createElement('select');
      bsel.style.cssText='background:var(--bg-2);border:1px solid var(--line-2);color:var(--ink-2);border-radius:6px;font-size:11px;padding:3px 6px';
      [['','vs \u2026'],['NIFTY 500','vs Nifty 500'],['NIFTY 50','vs Nifty 50']].forEach(function(o){
        var op=document.createElement('option'); op.value=o[0]; op.textContent=o[1]; bsel.appendChild(op); });
      bsel.onchange=function(){ if(bsel.value){ addCompare(bsel.value); bsel.value=''; } };
      cmpBar.appendChild(bsel);
      // hand the whole set to the full Compare tool (cap 12) \u2014 \u00a7M.2
      var oc=E('a','font-size:11px;color:var(--ink-2);border:1px dashed var(--line-2);border-radius:6px;padding:3px 8px;cursor:pointer;text-decoration:none','Open in Compare \u2192');
      oc.onclick=function(){ var cur=new URLSearchParams(location.search).get('sym')||'';
        var syms=[cur].concat(cmpReg.items.map(function(c){return c.sym;})).filter(Boolean).join(',');
        location.href='/dash/compare?sym='+encodeURIComponent(syms); };
      cmpBar.appendChild(oc);
  ...
  function makeDraw(pc, series, host, bar, getRows, getFut){
    ...
    // \u00a7N (M4 inc-2): pinned tools + MRU + 'All tools \u25be' grouped dropdown.
    var GROUPS=[['Lines',['trend','hline']],['Shapes',['rect']],['Fibonacci',['fib','fibext']],['Annotate',['text']],['Measure',['measure']]];
    function loadPins(){ try{ var v=JSON.parse(localStorage.getItem('pv3pins')||'null'); if(v&&v.length)return v; }catch(e){} return ['trend','hline','fib']; }
    function savePins(p){ try{ localStorage.setItem('pv3pins',JSON.stringify(p)); }catch(e){} }
    var pins=loadPins();
    function mruGet(){ try{ return localStorage.getItem('pv3mru')||''; }catch(e){ return ''; } }
    function mruSet(k){ try{ localStorage.setItem('pv3mru',k); }catch(e){} }
    function toolDef(k){ for(var i=0;i<TOOLS.length;i++){ if(TOOLS[i][0]===k) return TOOLS[i]; } return null; }
    var _setTool=setTool;
    setTool=function(k){ _setTool(k); if(k&&pins.indexOf(k)<0){ mruSet(k); buildToolRow(); } };
    var toolRow=E('span','display:inline-flex;align-items:center;gap:4px');
    bar.appendChild(toolRow);
    var ddBtn=tbtn('cursor:pointer;font-size:11px;color:var(--ink-2);border:1px solid var(--line-2);border-radius:5px;padding:3px 8px','All drawing tools, grouped \u2014 \u2605 pins a tool onto the rail','All tools \u25be');
    var dd=E('div','display:none;position:absolute;top:100%;left:0;z-index:30;margin-top:4px;min-width:190px;background:#0e1320;border:1px solid '+C.border+';border-radius:8px;padding:6px;box-shadow:0 6px 20px rgba(0,0,0,.5)');
    ddBtn.onclick=function(){ dd.style.display=(dd.style.display==='none')?'block':'none'; };
    function buildDD(){ dd.innerHTML=''; GROUPS.forEach(function(g){
        dd.appendChild(E('div','font-size:10px;color:var(--ink-3);text-transform:uppercase;letter-spacing:.4px;margin:4px 2px 2px',g[0]));
        g[1].forEach(function(k){ var t=toolDef(k); if(!t)return; var row=E('div','display:flex;align-items:center;gap:6px;padding:3px 4px;border-radius:5px;cursor:pointer');
          var pin=E('span','cursor:pointer;font-size:12px;color:'+(pins.indexOf(k)>=0?'#f0b429':'var(--ink-3)'),pins.indexOf(k)>=0?'\u2605':'\u2606');
          pin.title=pins.indexOf(k)>=0?'Unpin from the rail':'Pin onto the rail';
          pin.onclick=function(ev){ ev.stopPropagation(); var i=pins.indexOf(k); if(i>=0){pins.splice(i,1);}else{pins.push(k);} savePins(pins); buildToolRow(); buildDD(); };
          var lbl=E('span','font-size:12px;color:var(--ink)',t[1]+'  '+t[2].split(' \u2014')[0]);
          row.appendChild(pin); row.appendChild(lbl);
          row.onclick=function(){ setTool(k); dd.style.display='none'; };
          dd.appendChild(row); }); }); }
    function buildToolRow(){ toolRow.innerHTML=''; btns={_edit:btns._edit,_mag:btns._mag,_cfx:btns._cfx,_hide:btns._hide};
      var shown=pins.slice(); var m=mruGet(); if(m&&shown.indexOf(m)<0&&toolDef(m)){ shown.push(m); }
      shown.forEach(function(k){ var t=toolDef(k); if(!t)return; var b=tbtn(baseBtn(tool===k),t[2],t[1]);
        b.onclick=function(){ setTool(k); }; btns[k]=b; toolRow.appendChild(b); });
      toolRow.appendChild(ddBtn); paintTools(); }
    var ddWrap=E('span','position:relative;display:inline-block'); ddWrap.appendChild(dd);
    toolRow.appendChild(ddBtn); bar.appendChild(ddWrap);
    buildDD(); buildToolRow();
