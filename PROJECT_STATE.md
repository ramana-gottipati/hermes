...
**NEXT:** **DEPLOY** — `scp` the union `dashboard.py` + the `src/pat/` package → restart `hermes-api` → smoke-test all routes 200 (the last step); then the micro-viz design pass (Phase 6). (Glossary + tab + 6 avatars + explain + all 3 data flows + pt14 + Gemini engine + pre-deploy hardening all shipped this session.)
