...
- **Validated but DEFERRED to the morning briefing** (data earns the chart, but a live page waits): Sector Fundamental Regime Map (tag coverage ~322 syms — build once tags broaden) · ~~the features Anatomy Fingerprint~~ **✅ SHIPPED as `/dash/move-anatomy` (see above)** · quality-tier migration alluvial (veto-frame, returns flat across tiers) · ownership DII/FII drift · SLB short-interest (expiry-roll artifact + ~1mo) · 22y seasonality calendar. `stock_oscillators` diagnosed = orphaned one-shot (PK=symbol, ran once 2026-06-21; drop or wire nightly).
