...
│       ├── flows.py                                  ← read-only SQL templates compiled from chips (accumulation + RS flows live)
...
| **`/dash/pat?flow=&explain=&q=&sector=&strength=&entry=&align=`** | **D55 — "Pat", the natural-language guided-search tab.** Persona + 6 selectable Indianised avatars (localStorage). **Explain-a-metric** (search/tap the glossary, family-grouped, related-link chaining) + two live **data flows** — **Accumulation-setups** (strength/entry/sector chips) and **RS-leaders** (strength/sector/strong-in-strong chips) — each compiling to a read-only parameterized SELECT over `stock_signals`, rendered data-first (raw values beside pills) on the house **`table.dt`** grid (click-sort / filter / CSV-export, via the shared `_DT_JS`). Fundamentals flow announced, not yet built. Free-text uses the deterministic `find()` fallback until the Gemini engine lands. **Gemini-Flash-only, never Claude.** `src/pat/web.py` + `flows.py` | **D55** |
...
**NEXT:** the **fundamentals** data flow (note: `fundamentals` is a per-symbol cached snapshot — no `trade_date`/`primary_sector`, sparse coverage — so it needs care, not copy-paste) → the Gemini free-text engine (+ optional global ask-bar; returns only (flow, params) keys, never SQL; never-Claude provider check); tighten `pt14` from `scoring.py`. (RS-leaders flow shipped same session — D55 Phase 1c.)
