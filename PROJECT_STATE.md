...
> **Last updated:** 2026-06-18 (session 18 — **D43** DVPT accumulation/distribution **CHARACTER** (Positioning pillar, item 1): delivery is side-blind, so a 3-axis read — WHO (breadth) · WHICH-WAY (adjusted-price direction) · CONTEXT (52w location) — into 8 new `stock_signals` fields + a derived `accum_character` label, surfaced on `/dash/stocks` (Character + Total-deliv columns + Accum/Distrib pills), the stock page (character panel), Home (Stealth-accumulation board), and Telegram (`/dvpt` 🧭 block + `/triggers accum|distrib`). Prior: session 17 — D33b/D33c (RS pillar COMPLETE) + **D41** strategy-surface redesign Phase 1; **D42** equity-only scanners via the NSE `EQUITY_L` allowlist. Built on the session-16 D33a/D38/D39/D40 base.)
...
| `/dvpt TICKER [days]` | Delivery Value Per Trade institutional-flow signal: today vs power baselines + history + institutional price zones + **🧭 Character block (D43)** — accum/distrib label, total delivery ₹, deliv %, up/down split, breadth, 52w-high distance | ₹0 |
...
| `/dash/stocks?period=d\|w\|m` | Stock hub — search + layered-DVPT screen + filter pills (SS/A+/⚡ATH/🟢Discount/🔥Near-break + **🟢Accumulation/🔴Distribution, D43**) + watchlist + `?sector=` filter. Daily table now carries a **Character** column + **Total delivery ₹Cr** column (D43). **D41: Daily/Weekly/Monthly toggle** — w/m roll up the last 5/22 trading days (days-fired + peak rank + latest-day character, on-read) so a mid-window spike isn't missed; factor-index `?sector=` shows a graceful "see ratio chart" empty-state | D38/D41/D43 |
...
| `/dash/stock?sym=X` | Per-stock — adjusted candle + DVPT/delivery charts, DVPT inertia vs every baseline, D31 institutional price zones, **D43 Accumulation-character panel** (label + plain-English read + up/down delivery bar + WHO row + 52w-high distance + ⚠️ distribution-while-high warning), pt14 snapshot, RS vs broad+sector, auto-READ | D33-web/D36/D43 |
...
B8. **Positioning pillar — deepening, item by item.** ✅ **Item 1 (D43): accumulation/distribution CHARACTER** — SHIPPED (session 18). **Item 2 (NOT built) — NSE bulk & block deals direction feed:** the only public source that names buyer/seller + side (trades >0.5% of equity is reported with the counterparty and BUY/SELL). It is the lone GROUND-TRUTH for the WHICH-WAY axis that delivery data can only infer from price — ingesting it would let us *confirm* (not just infer) accumulation vs distribution on the names where a block printed. Free daily CSV from NSE. Not ingested today; this is the natural next Positioning item. Other candidate items: promoter/FII/DII quarterly-holding deltas, F&O OI/PCR cross-check for the largest names.
...
Verification: all 4 modules `py_compile` clean; the label rule + metric/anomaly logic unit-tested synthetically (markup⇒ACCUMULATION, quiet base⇒ACCUMULATION/absorption, broadening-near-highs⇒DISTRIBUTION, split-day excluded from the skew, all boundaries pass); db migration adds the 8 cols + index; dashboard routes `/dash`, `/dash/stocks` (d/w/m), `/dash/stock` return HTTP 200 (local DB empty). **VPS deploy + full `--backfill-triggers` (real-data eyeball on known names, character-distribution sanity, populated-route 200s) — see deploy note in this entry / next session if mid-flight.** Future Positioning item flagged: NSE bulk & block-deals direction feed (B8).
