...
> **Last updated:** 2026-06-18 (session 18 — **D44** value-weighted institutional **KEY PRICE** + multi-horizon near-key entry + ticket-size/turnover-surge + a one-screen **`/dash/workbench`** (additive, zero regression on existing columns); built on **D43** DVPT accumulation/distribution **CHARACTER** (3-axis WHO/WHICH-WAY/CONTEXT → `accum_character`). D44 refines D31's flat `avg_close_p*` zones — the big institutional day now dominates the value-weighted cost line, priced at the day's avg_price. Surfaced on `/dash/stock` (key-price section), `/dash/stocks` (🎯 near-key pill + Workbench link), `/dash/workbench`, and `/dvpt` (📍 block). Prior: session 17 — D33b/D33c (RS pillar COMPLETE) + **D41** strategy-surface Phase 1; **D42** equity-only scanners. Built on the session-16 D33a/D38/D39/D40 base.)
...
Verification: 4 modules `py_compile` clean; synthetic unit test PASSES the value-weighting spot-check (biggest power day cheapest ⇒ key 82.95 vs equal-weight 117.0) + asymmetric band + ticket/surge; dashboard routes (incl. `/dash/workbench`) HTTP 200. **Regression gate (existing columns byte-identical) + `--backfill-keyprice` + real-data spot-check run on the VPS — see deploy note.**
