...
- **Deferred (hand to the S-C education lane):** the 5 tracker demo pages (tracker_gate lane) + `wolfe_trades._bottom_line` fold, then item 3 (`gloss()`/`?q=`) → item 5 (nav de-metaphor). Consider extending the gate to also require `gloss()` once item 3 lands (checklist §3 names it too). Harness TIL: `mcp__ccd_session_mgmt__list_sessions` is the fast way to SEE the parallel lanes (titles) at boot — run it the moment `git status` shows edits you didn't make, before guessing which lane owns what. Commits `b315e4a` (gate) · `3788a4f` (screen2) · `ed442f4` (batch 2) · `e3e2550` (batch 3) · `2db52bd` (batch 4).
