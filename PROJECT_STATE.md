...
B7. ✅ **THEME TAGS (D63) — SHIPPED + DEPLOYED (session 33).** Multi-label thematic layer: `company_tags`/`company_about` + `theme_tags.py` (355 deterministic tags seeded) + 4 UI surfaces (stock chips · `/dash/themes` + `/dash/theme` · screener column+filter · index participants Themes column via shared `_participants_table`) + `/dash/tags-review` (approve/add) + weekly reseed timer. **Closed the run-book open items:** signed-MEP per participant on the index page ✓, "accumulating only" filter ✓, Compare link on sector index pages ✓. **Still open:** (a) the **Haiku proposer is gated** on a `company_about` corpus — `screener.fetch_company` now harvests it best-effort, so it fills on the Screener cadence; once non-empty, run `python -m src.automation.theme_tags --propose` then approve at `/dash/tags-review`; (b) **ask Ramana what "DBP" meant** (run-book §4 — unresolved; he said "still holding that position in DBP today"); (c) **size-index inline RRG/Mansfield depth** for the exact names `Nifty Midcap 150`/`Smallcap 250` needs the RRG-stream to compute those numerators (coordinate with that stream).
...
- **NEW routes:** `GET /dash/themes`, `GET /dash/theme?tag=`, `GET /dash/tags-review`, `POST /dash/tags`. **UNCOMMITTED at time of writing → committing my files this session** (see Decision **D63**). Still-open: ask Ramana what **"DBP"** meant; size-index inline RRG depth (RRG-stream change); the Haiku proposer awaits a corpus.
