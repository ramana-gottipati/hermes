...
### ⭐ NEXT (designed 2026-07-12, NOT built) — Wolfe "Open trades — remaining ROI" filterable view
Full buildable spec: **`docs/wolfe-open-trades-PLAN.md`**. Ramana converged the design over a long session, then asked to wrap before the build. Gist: a NEW **additive** view listing every **OPEN** winner-profile trade (point 5 printed, EPA not hit) at **any age** (the fresh-≤15 scanner hides ~686 of them), ranked by remaining ROI. **run% / risk% / R:R measured from CMP** + the full trade row: `sym · dir · sector · size · liquidity/deliv · status · CMP · entry zone · SL · T1 · EPA · run% · risk% · R:R · Q · RS · age · ✓edge`. **9 top-of-page filters** (Size ← `stock_index_membership` · Sector ← `company_tags` multi-label · Direction · Max-age · Min-Q/"top-20" · Min-room% · Status[in-zone] · **Min-liquidity** · Min-R:R) — the persist/script must honor the SAME params, not just the UI. The **liquidity** column+filter systematizes the TIRUPATIFL-untradeable lesson (its 98-day gap made it un-fillable). **Additive — do NOT touch detection / §A / §B / `winner_scan` / the crossing rule / point-4 reconciliation.** ⚠ `wolfe.py`/`wolfe_view.py` are co-edited + the VPS copies are git-untracked → deploy the union + `.bak`, `curl -sL` verify. Open micro-confirms for the builder: sector from `company_tags` (recommended) vs `primary_sector`; keep the full column set (Ramana leaned yes).
