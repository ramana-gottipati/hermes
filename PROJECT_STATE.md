...
- **Remaining to light it up (all additive, no code-gate change):** (1) backfill NSE **index history back to ~2004** (primary source) so broad-index month cells reach N≥15 — THE unblock; (2) P2 **stock layer** (`--scope stock` + earnings-cadence mask + event triangles + RS ladder/drill/confluence); (3) forward-outlook strip + 25y-stack UI polish. Commit `<SEAS-HASH>`.
