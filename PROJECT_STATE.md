...
Surfaced: NEW `/dash/conviction` (sortable/filterable/exportable `.dt` + 🎯 near-key / ★ quality filter pills) + a ⭐ Conviction preview board on Home (right after the Strategies hub — the headline) + the `/conviction` Telegram command (same helper). Realizes the Conviction-shortlist half of D41's Phase-3 roadmap. Verification: 3 modules `py_compile` clean; `conviction_shortlist()` imports + returns a list; dashboard routes (`/dash`, `/dash/conviction`) HTTP 200. **Deploy (scp/pull + restart hermes-api; no backfill) + real-data check — see deploy note.**
...
Next: D41 **Phase 2** (materialised `weekly_signals` + membership for India Defence etc.), **Phase 3** (saved-screener/query-builder — **the Conviction-shortlist half SHIPPED as D45**). Then B5/B6, Telegram unblock.
