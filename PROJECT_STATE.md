...
**Deploy (when Ramana asks):** scp `src/core/db.py` + `src/web/dashboard.py` → VPS, restart `hermes-api` (the `CREATE TABLE IF NOT EXISTS` runs on boot). *(Not committed/deployed at write time — commit ONLY these two files + the 3 docs; the tree also carries the dormant `patearn.py` diff + held `mtf_signals.py` — do NOT sweep them in: `git add` explicitly, `git diff --cached --name-only` before commit.)*
