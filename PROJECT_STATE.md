...
- **Deferred (hand to the S-C education lane):** trim PENDING as pages land item 2's remaining tail (dossier-strip/`concalls` in forked `dashboard.py` + `wolfe_trades._bottom_line` fold), then item 3 (`gloss()`/`?q=`) → item 5 (nav de-metaphor). Consider extending the gate to also require `gloss()` once item 3 lands (checklist §3 names it too). Harness TIL: `mcp__ccd_session_mgmt__list_sessions` is the fast way to SEE the parallel lanes (titles) at boot — run it the moment `git status` shows edits you didn't make, before guessing which lane owns what. Commits `b315e4a` (gate) + `__PENDING__` (screen2 scaffold).
