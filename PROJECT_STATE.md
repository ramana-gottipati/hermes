...
- **⚠ DEPLOY DEFERRED (deliberate):** not deployed this session — (a) the tree is mid-churn (defer to a quiet, non-mid-day window per AUD-95), and (b) it is the "makes it send" step, so it warrants a controlled deploy + then setting `HERMES_ALERT_DM=1`. Deploy recipe: clean-scp the 1 new module + the anchored `signal_events.py` block (fork-check md5 vs HEAD-base first) · remote import-test · writer-safe `hermes-api` restart · `--push --dry-run` on the box to preview · then `HERMES_ALERT_DM=1` in `/opt/hermes/.env` to arm. **Harness TIL:** an `EnterWorktree` isolated checkout would have spared me the repeated cross-absorption fights this session (a sibling's `git add -A` swept my staged files once) — the mitigation that held was `git commit -- <explicit paths>` (atomic, no separate `add`).
