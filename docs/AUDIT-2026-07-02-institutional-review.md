...
**AUD-06 [P1] D31 price zones, D44 key prices and hot-day averages computed on RAW closes across ≤360d windows** — `FIXED `4548a01` (2026-07-10 D107; golden test tests/test_signals_adjusted.py; VPS backfills queued post-chain)`
...
**AUD-07 [P1] Backfill and nightly disagree on hot-day definition (22-of-22 uncapped vs 15-of-22 capped)** — `FIXED `4548a01` (2026-07-10 D107; golden test tests/test_signals_adjusted.py; VPS backfills queued post-chain)`
...
**AUD-08 [P1] Revised insider filings double-count promoter flow (amendments never supersede)** — `DONE S77 (deployed+verified)` (insider_events.py `_supersede()`: collapse each natural-key group with an amendment to its latest; distinct same-day trades untouched. Live VPS assertion: net_90 270→170, cluster 2, distinct-trades 140. Golden test tests/test_insider_supersede.py)
...
**AUD-11 [P1] Corp-action fallback rescales entire history on genuine >30% crashes (F&O names)** — `FIXED `4548a01` (2026-07-10 D107; golden test tests/test_signals_adjusted.py; VPS backfills queued post-chain)`
...
**AUD-12 [P1] Historical rs_rank percentiles are survivorship-biased (today's equity list applied to all past dates)** — `VERIFIED-REAL + MATERIAL (S123, 2026-07-13) → routed to codex D2-F1 (codex-lane-owned; do NOT double-fix)`. The "verify first" is done: **CONFIRMED real** — `stock_rs.py:83` `_LIQUID_FILTER` = `AND s.symbol IN (SELECT symbol FROM nse_equity_list)`, and `nse_equity_list` is the CURRENT snapshot (what equity_list.py refreshes); it is applied inside `_rank_sql_for`'s `PERCENT_RANK() OVER (PARTITION BY trade_date …)` (line 257), so every HISTORICAL cross-section is ranked over survivors only → the stored `rs_rank` percentiles are survivorship-biased. **MATERIAL, not merely latent:** the stored historical `rs_rank` is consumed by the explosive-move factor studies (`research/explosive_moves/combo_test.py:68` selects `rs_rank`; also `panel_build.py`/`features.py`) — so the bias can inflate the very momentum-factor screens the ledger reports (the AUD-22 attribution itself is survivorship-aware via factor_zoo terminal-return booking, but the univariate `rs_rank` screens inherit the bias). **Fix recipe (for the owning lane):** replace the `nse_equity_list` predicate with a PIT `security_master.universe_on(trade_date)` join (it EXISTS, `security_master.py:322`, ignition already uses it) in the all-dates rank pass, then re-run the rank backfill on the VPS. **This is exactly codex `D2-F1`** ("sector map + `rs_rank` write current snapshot across full history", adjudicated **AGREE-FIX**, Theme A) — `stock_rs.py` is codex-lane-owned and the fix needs the coordinated VPS rank re-run (their bias-verify-restate track), so it was NOT edited here to avoid a collision + respect the Codex↔Claude convergence governance. Original marker below.
- ORIGINAL: `OPEN`
...
**AUD-14 [P1] NSE throttle (403/429/5xx) recorded as holiday in bhavcopy + 5 sibling fetchers; run_recent never re-attempts lost days** — `CLOSED (S123, 2026-07-13 — all 6 fetchers)` — NEW shared `src/automation/fetch_retry.py` (`RetryableFetchError` + `raise_if_retryable`, centralizing the fno_oi CL-RS-13 pattern). **bhavcopy (the master feed — its hole corrupts every calendar-window signal): FULL fix** — `_try_fetch` raises on 403/429/5xx/network (throttle ≠ holiday); `ingest_date` leaves a throttled date UN-marked (retry) + wraps the optional MTO enrichment fetch; `run_recent` now scans the **FULL 7-day lookback** (was early-return on first success → a throttled day D was never revisited once D+1 landed); genuine no-data ≥3 days old gets a **row_count=0 holiday sentinel** (so a missing `bhavcopy_dates` row now means only throttled/pending — a visible gap), while a recent no-data day stays pending (may still post). **indexes + participant_oi (the other per-date feeds): ported** — `_try_fetch` raises; the caller returns a clean "blocked — retry" (indexes' `fetch_and_store`; participant_oi's `run_today`, matching fno_oi; its `run_backfill` already caught `Exception`). 6 pytest regressions (`tests/test_aud14_fetch_retry.py`); all imports clean. **Sibling follow-ups:** `deals.py` **✅ CLOSED via AUD-53** (`742b12f` — warm session + 3× backoff + 2nd timer) · `corp_actions.py` **✅ CLOSED (S123, 2026-07-13)** — `fetch_window`'s blanket `except Exception: return None` (which swallowed the retryable) restructured into a retry loop: a throttle (403/429/5xx/network) now retries 3× with backoff before returning None (a genuine non-200/parse error still returns None immediately for the breaker); 5 pytest regressions. `equity_list.py` **✅ CLOSED (S123, 2026-07-13)** — already self-healing (empty fetch → keep the prior allowlist + AUD-43 shrink guard), so a behaviour-preserving port: `fetch_equities`/`fetch_etfs` now LABEL a 403/429/5xx/network as a throttle distinctly (so a persistently-stale universe is diagnosable as rate-limiting) while still returning `[]`; 5 pytest regressions. **→ ALL 6 fetchers now share the `fetch_retry` taxonomy; AUD-14 FULLY CLOSED.**
...
**AUD-22 [P1] Research replication stack bypasses the provenance PIT layer (gates on the leaky modeled report_date)** — `CLOSED (S123, 2026-07-13)` — `attribution.py` / `factor_zoo.py` / `gate_study.py` `load_frame` now route fundamentals through `src.automation.fundamentals_asof.load_symbol_history` (the provenance EFFECTIVE-date map: real BSE filing date, else the conservative calibrated lag), so `latest()`'s `rd <= asof` gate is no-leak; the `attribution.py:8` "PIT" mislabels are corrected and the reproduce commands updated to `PYTHONPATH=/opt/hermes:/opt/hermes/research`. Verified the eff-map ENGAGES in the research venv (RELIANCE FY24 `2024-06-29`→real `2024-04-22`). **Re-ran attribution (audit's prescribed action, "ledger entry either way") — the residual-α conclusion HOLDS + strengthens:** RISKADJ residual α **+7.3%/t=1.99 → +6.69%/t=1.80** (still fails the t≥3 selection bar), WML share **51%→58%**, Fama-MacBeth momentum λ t 3.36→3.30, DSR 0.966→0.963, PBO 0.34→0.357 — correcting the leak made "momentum is BETA not selection skill" MORE clear, not less. Recorded in `docs/predictive-attributes-findings.md` (headline). (`strategy-ledger.md` line-724 fold DEFERRED — that file is codex-lane-owned per their multi-session guard.) **REMAINDER CLOSED (S123 follow-up):** a 4th research module also bypassed the PIT layer — `research/cci/gate_residual_alpha.py._load_fund_frame` read `fundamentals_history` directly and gated ROCE/debt on the leaky modeled `report_date`; now routed through `fundamentals_asof.load_symbol_history` too (kept `_fund_con()` as the availability probe; frame shape + metric keys identical, so a clean drop-in). **Committed (`7ed6f95`) + verified LOCALLY** (AST + import; the `load_symbol_history` drop-in is proven by attribution/factor_zoo, same frame shape). **VPS re-run BLOCKED (deploy gap, not a code bug):** the box's `research/cci/` is an OLDER snapshot — its `common.py` lacks `MIN_RESOLVED_ASOF`, which the newer HEAD `gate_residual_alpha.py` already imports — so it ImportErrors there; bringing `research/cci/` to HEAD is a separate coordinated deploy (`common.py` is shared by other modules). Box restored to its consistent older version (import re-verified). The fix applies once `research/cci/` is deployed to HEAD; CCI is descriptive-only regardless, so the Gate-B verdict is FAIL/UNSCORED either way.
...
**AUD-25 [P1] Feed-liveness monitoring covers ~4 of 12 feeds; regime guard reads undated rows** — `MOSTLY CLOSED (S123, 2026-07-13)` — coverage had already grown to 8 event feeds + `chk_derived_liveness` (stock/mep/cpr/index signals vs the bhav clock) since the audit; this session added the **explicit two sub-items**: (a) **`chk_regime_guard` date-freshness guard** — a frozen Nifty tape no longer yields a plausible-but-stale 200DMA verdict (`regime UNKNOWN — index feed stale`); the verdict is now dated `(as of …)`; (b) **`chk_feed_freshness` +news (`sent_news.sent_at`, 7d) +concalls (`concalls.result_filing_dt`, 120d)** → **10 feeds**. 4 pytest regressions (`tests/test_data_quality_liveness.py`) + module selftest green; live-verified read-only on prod (feeds all `ok`, regime dated). **Residuals DEFERRED (with reasons):** fundamentals_history / shareholding_history recency — both carry FORWARD-dated `report_date`s so `MAX(report_date)` reads perpetually fresh and would HIDE a death (needs an `ingested_at` column; integrity already covered by `chk_fundamentals_dates`+`chk_leak_and_demodel`); and the holiday-tolerant bhavcopy trading-day-gap check — blocked on **AUD-14**'s holiday sentinel (the calendar-day `age>4` in `chk_market_freshness` is the current proxy).
...
**AUD-28 [P1] The documented deploy script would actively regress live units (bidirectional git/VPS drift)** — `CLOSED (S123, 2026-07-13)` — `setup-news.sh` no longer writes units from inline heredocs (which had drifted from the box — missing the hermes-concalls `ExecStartPre=-cci_pipeline --targets --ingest`, the bhavcopy chain drop-ins, the api bind override, the AUD-30/31/35 hardening drop-ins). The 9 heredoc blocks + the `systemctl daemon-reload/enable/restart/start` block (also an AUD-95 auto-start violation) were removed; unit management is DELEGATED to `scripts/install-systemd.sh --install` (AUD-27's canonical installer — source of truth `scripts/systemd/vps-live/`, copies verbatim + daemon-reload, NEVER enables/starts). Dormant units (nous-review, D145) are install-guarded by exactly that never-enable behaviour. `bash -n` clean; verified zero executed `systemctl` + zero heredocs; the captured concalls unit carries the `ExecStartPre` the heredoc dropped. Repo-only (the script is NOT run on the VPS — the deploy recipe stays scp + writer-safe restart); CLAUDE.md ban note updated to "fresh-bootstrap only, AUD-28 fixed".
- **Component:** deploy safety | **Reporter:** timer-topology.
- **Files:** `scripts/setup-news.sh:138-148` (heredoc missing the live `ExecStartPre=-cci_pipeline --targets --ingest` on hermes-concalls.service); reverse drift: committed `hermes-code-review.{timer,service}` absent on VPS (dormant by decision D145, formerly D68 — needs a disabled-by-default guard, not deletion).
...
**AUD-37 [P1] /v1 metering under-records: 500s never logged, bytes_out always 0, insert failures silently dropped — billing substrate not audit-grade** — `CLOSED (S123, 2026-07-13)` — `_observe` now wraps `call_next` in try/except and meters the 500 (bytes unknown) before re-raising to the RFC-7807 handler; `bytes_out` reads `Content-Length` (set on /v1's JSON, preserved through the wrapper) instead of the wrapper's absent `.body`; `record_usage` retries once on a lock then **logs** a dropped row (no longer silent); and `_any_problem`/`_http_problem` stamp X-Request-ID + RateLimit on error responses. 3 pytest regressions (`tests/test_v1_metering.py`) + the full v1 selftest green. **The deferred sub-item — a per-tenant daily/monthly quota beside `rate_check` — is now ALSO built** (the entitlement substrate for tiered data plans): nullable `daily_quota`/`monthly_quota` on `v1_tenants` (NULL = unlimited, idempotent migration), `metering.quota_check` (counts SERVED requests over the usage log, sargable range scan, fail-OPEN + zero cost when unset), enforced in `auth.require_scope` (429 with a quota-specific message), + a `keys.set_quota` helper / `--set-quota` CLI. 6 more pytest regressions.
...
**AUD-38 [P1] PIT — the product wedge — is not on the /v1 contract (5 of 6 endpoints latest-row-only)** — `DONE S96b (2026-07-10): as_of on credibility (month-end knowable rule, knowable_from stamped) + attention (last-batch-on-or-before resolver); PIT contract in OpenAPI (app description + per-param); malformed as_of → RFC-7807 422; regression tests tests/test_v1_pit.py + selftest 5b; live-verified on ICICIPRULI (97-pt series). P-05 unblocked.` `REFINED S100 (D104): knowable clock upgraded to two-tier — the REAL concall/transcript event date (max(concall_dt, transcript_publish_dt); 16,208 rows carry one) when captured, month-end only as the fallback; result_filing_dt refused (leak direction); rows stamp knowable_basis EVENT|MODELED, pit.knowable_rule names the applied rule; /universe as_of strictly validated; +5 tests.`
...
**AUD-40 [P1] Pat eval battery wired to no gate, timer, or CI — the accuracy layer never runs anywhere** — `DONE S77 (wired) → S83i (baseline cleared + enforced + pager-verified)` (eval_set.py: --gate mode, exit-non-zero on any fail, UTF-8 stdout (charmap crash fixed); accuracy layer runs against prod DB. **S83i (9ed6aa4) cleared the residual — the 6 surfaced misses FIXED at root: (1) parse_fallback now extracts a numeric P/E threshold ("under PE 15" → deep-value filter); (2) detect_compare stop-set widened so "explain vs market/sector" reach the explain path (was parsed as compare(EXPLAIN, MARKET)); (3) the one >45-char non-parenthetical glossary term renamed ("Financials & the pt14 score", slug unchanged). Battery 100% green on the VPS: ROUTE 64/64 · EXPLAIN 558/558 · COMPILER 31/31 · HALLUCINATION 8/8 · ACCURACY 10/10. Gate hardened for env-independence: CATALOG = printed coverage, NOT gated (deterministic-floor metric, <100% by design); ACCURACY skips a missing table/column rather than failing. gate-0.5 flipped STRICT-by-default (PATEVAL_STRICT=0 = documented WARN escape hatch). NEW nightly `hermes-pateval.{service,timer}` (08:15 UTC) → `OnFailure=hermes-alert@` = the AUD-26 Telegram pager; a forced-failure test confirmed the DM fires end-to-end (paged chat 282907906) and the gate self-restores green.**)
...
| AUD-53 | P2 | Deals feed: current-day-only, no retry, no cookie warmup, no liveness — one 403 evening = permanent loss | `deals.py:86-97` | **✅ CLOSED (S123, 2026-07-13):** `_warm_session()` primes anti-bot cookies (insider_events pattern) + `_fetch` does 3× exponential-backoff retries on a throttle (403/429/5xx/network via `fetch_retry`) shared over one session across bulk/block/fii-dii; a **second late-evening trigger** added to `hermes-deals.timer` (16:30 UTC = 10 PM IST, idempotent re-fetch via the UNIQUE constraint); liveness already covered (`bulk_block_deals` is in `chk_feed_freshness`, AUD-25). 5 pytest regressions. | S | finder-only |
...
| AUD-81 | P2 | No .gitattributes: CRLF tree feeds the scp deploy to Linux; LF is a human-memory rule | repo root | `*.py/*.sh/*.service/*.timer text eol=lf` + renormalize; or deploy via `git show HEAD:file` | S | **DONE S77** — `.gitattributes` added (`* text=auto eol=lf` + explicit source/deploy types LF, `.bat` crlf, binaries protected). Index was already all-LF so git normalizes the 209 CRLF worktree files on-the-fly — no risky renormalize, no `git status` flood. **Proven: PROJECT_STATE.md whole-file churn is GONE.** Optional cosmetic worktree re-checkout deferred to a quiet window. |
...
| AUD-87 | P2 | Working-notes voice on client-facing glossary ("I (Claude) introduced…", stale examples) | `metrics-glossary.md:31,32,40,42` | one editorial pass: neutral product voice, keep honest caveats as product limitations | S | **DONE S77** (voice neutralised w/ AUD-20) |
| AUD-88 | P2 | Pat mis-states its own lineage: fundamentals described as live Screener pull; no freeze/XBRL disclosure | `pat/glossary.py:611+,640-648` | update entries: frozen legacy snapshot + NSE XBRL for new periods + broadcast-timestamp PIT story (Guardrail #8 disclosure duty) | S | **DONE S77** (Screener→XBRL migration disclosed, deployed) |
...
| P1 | Eval battery wired to no gate/timer/CI | pat/eval_set.py:539 | **C** → DONE S83i (green+strict+pager) | AUD-40 |
...
| **AUD-101** | UNBLOCKED 2026-07-10 (AUD-04 landed `c948c3f`+`a207c99` S77) | ETag/Cache-Control validators now key off AUD-04's cache stamp / MAX(trade_date). **CONFIRMED-FEASIBLE, buildable now.** |
