...
**G · Confluence zone NEAREST the current price** (Ramana 2026-07-11) — judge the confluence zone closest to the **last traded price** (it starts at point 5 and tracks price up toward point 3 for a bull / down toward point 3 for a bear; the zones sit between p5 and p3, so once price clears p3 nothing is near → 0). If the LTP is near that zone (its band ±2%), score by its ratios: **not near any zone = 0** · a shallow confluence = **1** · zone involves **2.618** = **2** · involves **4.618** = **3** · involves **BOTH 2.618 and 4.618** (one from each leg's extension) = **4**. *(Supersedes the wrong "4.618 anywhere in the wave" version — the zone must be where the price actually is.)*
...
**Quality score = A + B + C + F + G + H + I + D** — a plain **points sum**, **max 27** (A4 + B3 + C4 + F4 + G4 + H3 + I2 + D3). Higher = the better pick. Point 1 is never averaged into B; point 5 is never scored on a fractal. No weighted-0-100 blend, no freshness. **✅ LIVE (D111, 2026-07-11)** — incl. 2.0 restored to the confluence ratios (`_FIB_R`, 1.414 kept).
