...
- **⚠ REMAINING SURFACE GAP (out of "wire the tape" scope — NOT done):** ETFs still can't be *added* to a chart — `/dash/compare` validates `sym` against `nse_equity_list` (2386 equities, no ETFs → GOLDBEES silently dropped), and `/dash/stock` uses its own inline D36 adjustment (open-item B5), not `adjusted_closes`. So the ETF-subdivision fix is correct-and-ready in the adjust LAYER but not user-visible until a surface accepts ETF symbols. The RESEARCH `load_factors` path (portfolio_mix, the gold leg) remains fully fixed.
