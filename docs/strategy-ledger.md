...
- **⚠ DISCLOSED, NOT FIXED (separate, out of this DQ scope):** the production `src/automation/adjust.py` path still cannot render a 100:1/50:1 GOLDBEES chart even with the row present — (i) NO production consumer wires the tape (`events=`/`price_ratios`) today, so it runs pure-fallback, and (ii) the fallback + `price_ratios`' `0.02 < v < 50` guards exclude r≤0.02. In practice the equity dashboard/RS/signals universe is Nifty-500, not ETFs, so the live impact is ~nil — but if a gold-ETF chart is ever surfaced it needs either the tape wired or the guard widened for the ETF class. The RESEARCH `load_factors` path (portfolio_mix, the gold leg) is fully fixed.
- **Provenance:** `scripts/backfill_etf_splits.py` (new) + box read-only verify. Design doc §7c marked RESOLVED; PROJECT_STATE S182 + Key file paths; carryforward DQ task closed. Guardrail #8 clean (NSE bhavcopy primary); classify_event(SPLIT)=None → no spurious `security_events` breaks.
