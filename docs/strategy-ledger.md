...
- **✅ SURFACE GAP CLOSED for `/dash/compare` (S182 follow-on):** `_picker_equities(conn)` now merges `nse_etf_list` (328 ETFs) into the compare picker + validation set (2386→2714, labelled `(ETF)`), so ETFs can be *added* to the overlay and render tape-adjusted. Verified: GOLDBEES/QGOLDHALF/SETFGOLD validate + 0-cliff `_stock_levels`; live chip + picker `(ETF)` entry, route 200. Remaining: `/dash/stock` single-symbol pages still use the inline D36 copy (open-item B5) — a separate unification. The RESEARCH `load_factors` path (portfolio_mix, the gold leg) remains fully fixed.
