...
> **🔁 OOS RE-VALIDATED under the D111 §B rebalance — 2026-07-11 (`2545a91` doc · `3c54c8a` PROJECT_STATE · carry-forward + `[[wolfe-wave-strategy]]` memory). SUPERSEDES the June-26 live numbers below** (that entry = the PRE-D108-gate baseline, kept as history). Re-ran the committed `phase2_oos.py` + `phase3_betacontrol.py` (unchanged — they import production `wolfe`) READ-ONLY on the VPS archive under the CURRENT scoring (F 0-4, `_QUALITY_MAX`=27, point-4 live; from a VPS `/tmp` scratch + `PYTHONPATH=/opt/hermes`, `/opt/hermes` untouched). **① Filter UNCHANGED** — the 2004-14 decode-median fit **re-derives the IDENTICAL `D≤1 & p1≥2 & F≤2`** (the F 0-4 widening is neutral: F≤2 = zone gap >0.6%, `wolfe.py:329` confirmed). **② BULL edge intact** — test 2015-26 medNet **+4.4%** inclusive / +4.7% nifty500; residual **α +5.07** (CI [+2.0,+7.3] excl 0). **③ Placebo-gap NEGATIVE everywhere** (ALL −1.7% · BULL −0.5% · BEAR −2.2%) → edge = SELECTION not craft, REAFFIRMED. **④ BEAR now FAILS the primary (survivorship-aware inclusive) OOS `medNet≥0` bar → verdict IN-SAMPLE-ONLY** (−0.98%; medNet|mkt≤0 −2.1%, decaying) — nifty500 = SURVIVED (point-estimate, bear CI straddles 0). **⑤ SOFTER than the June baseline** (inclusive winner medNet **+2.14%→+0.81%**, BEAR **+1.03%→−0.98%**, n 1787→1591) — attributed (MEASURED) to the cumulative directed changes since June, **dominantly the D108 mandatory fractal gate** (removed ~⅓ of waves); **point-4 reconciliation confirmed NEUTRAL** (A/B: `_reconcile_point4`-disabled = +0.78% vs +0.81% enabled — confirms its "822→822" claim on OUTCOMES), phase1 `scrfix` `9594c6e` = comments/CLI-default only. **Descriptive-only UNCHANGED** (a re-validated score is not a validated strategy). Re-run recipe: scp the committed `phase{1,2,3}*.py` to a VPS `/tmp` + `PYTHONPATH=/opt/hermes`. Full record folded into `docs/strategies/wolfe-wave.md` §4+§8.
>
...
>
> **✅ BETA / REGIME CONTROL + PLACEBO — DONE 2026-06-26 (closes the skeptic's last OOS box). `research/wolfe_waves/phase3_betacontrol.py` (isolated, read-only; reuses phase2 fit/collect; benchmark = Nifty 500 `index_rows`, 2012+ covers the test window).** On the TEST-window (2015-26) winner-profile trades: (1) OLS trade_net ~ α + β·signed-market-return (per direction, bootstrap α-CI); (2) REGIME TELL = medNet conditioned on the market NOT in the trade's favor (signed_mkt≤0); (3) PLACEBO = raw signed H-bar buy/sell-hold of the same name vs the Wolfe net. **RESULT — the edge SPLITS BY SIDE:** **BULL = a genuine, regime-ROBUST long selection edge** — α +5.91 (CI [+4.03,+8.97]) and **+1.37% median even when the market FALLS** (n=181); clean pass. **BEAR = FAILS as a standalone scanner edge** — once regime is stripped the typical short is **−0.19%** (medNet|mkt≤0, n=741) and **DETERIORATING**: +0.92% (2015-20) → **−0.94%** (2021-26); only the MEAN/TAIL α survives (+3.06, CI [+2.47,+3.61], stable H=20/40/60) = a separate, placebo-unattributed SHORT-BASKET claim, NOT a scanner edge. **PLACEBO gap < 0 everywhere** (bear −0.59%) → the edge is in **SELECTION** (which name/dir/when), NOT the entry/stop/target trade-craft (the 0.3% stops bleed on daily noise; craft = risk-shaping, not edge). **PANEL:** skeptic — BULL passes the control cleanly; BEAR resolves the "pending beta-control" qualifier **DOWN to FAIL-for-bears-as-a-scanner** (governing metric for a scanner = median-in-adverse; it fails + decays). Ramana-proxy — trust BULL calls standalone; trust BEAR only **conditionally** (tape already against the name) or as small **tail** bets; placebo doesn't sting (selection was always the machine's job). **FINAL VERDICT: a regime-robust LONG selection edge + a regime-dependent/tail-only SHORT descriptor; edge = SELECTION not craft; STAYS DESCRIPTIVE — now LABELLED SPLIT BY SIDE.** Open (optional, cheap): the placebo MEAN/TAIL (only the median was reported) → does Wolfe add anything to the bear tail vs naive shorting. **✅ SPLIT-BY-SIDE LABELLING — DONE + LIVE 2026-06-26** on `/dash/wolfe/scan` (`wolfe_view.py`, isolated): the intro was reframed to the true-OOS + beta-control reality (dropped the now-stale "median +2.4% / bears +1.1% / 3-eras" claim); header reads "read by side"; **BULL** rows tagged **✓ edge** (regime-robust long selection edge), **BEAR** rows tagged **⚠ tail** (regime-dependent / tail-only — reliable mainly when the broad tape is already weak); still DESCRIPTIVE (not a buy/sell signal). Deployed + render-verified (health 200; **20 ✓ edge + 34 ⚠ tail = 54 candidates**); the VPS `wolfe_view.py` was IN SYNC with HEAD before overwrite (safety-diff clean → nothing clobbered). **NEXT (optional R&D, all gated): bull-focused shallower entry (capture the ~50% missed EPA moves) · the placebo mean/tail close-out · nightly-persist the scan.** Committed (isolated): `research/wolfe_waves/phase3_betacontrol.py` + `src/web/wolfe_view.py` + this run-book.
>
...
>
> **§C BACKTEST — PHASE 1 DONE + DECODED** (`research/wolfe_waves/phase1_tradesim.py`, isolated/read-only; Nifty 500 ≈ 90s/run, survivorship-inclusive control ≈ identical). His full trade-mechanics simulated (entry at the pt-5 confluence ±0.5% · SL zone-edge ∓0.3% · re-entry at the next/returned zone, ≤3 legs · T1=0.618∩0.618 book ⅓ · ride the ⅔ to the EPA at break-even · missed-entry accounting). Validated blow-by-blow — fixed two artifacts (zone re-churn; bar-low fills). **VERDICT: no proven market-neutral edge.** Net +8.8%/trade BUT median **−2%** net; a pure **tail game** (top 1% of trades = **58%** of profit, max **+5860%**). **Direction is the whole story: BULL +20% net (mostly BETA — long in a 20-yr bull market), BEAR +0.6% net (the beta-neutral test = ~no edge).** Lens **STAYS DESCRIPTIVE-ONLY** (the gate worked). The **Q-score INVERTS as a trade-filter** (low-Q trades better) because §B rewards far-EPA (D) + narrow-zone (F) — and the **DECODE proves those are the LOSER traits:** WINNERS (rode to EPA) = **strong point-1 (p1=2 vs 1), WIDER zone (F=2 vs 3), CLOSER reachable EPA (D=1 vs 2), ONE clean entry (1 leg vs 3), shorter wave**; the score selects the opposite. **Missed entries = 50% of EPA-reaching moves** (entry too deep). **✅ WINNER-PROFILE REBUILD — DONE + OOS-VALIDATED + KEPT AS A SCANNER (2026-06-25):** the validated edge = **reachable EPA (D≤1) + strong point-1 (p1≥2) + not-narrowest zone (F≤2)** — the OPPOSITE of ranking by the §B total (whose far-EPA D-preference selects the no-edge lottery). It flips the median trade from −2% to **+2.4% net (hit 59%, EPA-reach 57%)** and — the prize — **the beta-neutral BEARS turn positive (+1.1% median, +3.0% net)**, CONSISTENT across 3 independent eras (2004-11 / 2012-18 / 2019-26 → winner medNet +1.5/+2.5/+2.4%; bear medNet +0.8/+1.3/+1.1%). The EPA-distance (D) is the whole axis (p1/F refine); decode-median thresholds (no sweep) → low overfit. **Per Ramana, kept ONLY as a SCANNER** — `research/wolfe_waves/scanner.py` lists the current universe's FRESH winner-profile setups (entry zone · 0.3% stop · T1 · EPA target · up% · "IN"=actionable-now; `--asof YYYY-MM-DD` PIT-replays any date). NOT a trading/chart-signal build. **Open (optional): a true OOS derivation (fit 2004-14 / test 2015-26); a shallower entry to capture the 50% missed; nightly-persist the scan.** Full methodology + his verbatim rules = memory [[wolfe-backtest-methodology]].
