...
| 5 | Glossary keys | new metrics get entries in **`docs/metrics-glossary.md`** (the source — `src/web/glossary.py` parses it) and surface through the popovers AND Pat's explain corpus. **Machine-enforced:** `tests/test_pat_coverage.py::test_screener_columns_are_glossary_backed` — every Screen+ metric column carries a glossary key (or an explicit `''` opt-out) |
| 6 | Pat registration | at minimum a **nav-answer** (Pat can name + link the page for a matching question); data flow if the page is a table/screen. **Machine-enforced:** `tests/test_pat_coverage.py` — every routed lens must declare Pat coverage (DATA / EXPLAIN / NAV) or the build fails. The three self-feeding knowledge sources + the same-commit rule are the canonical **`docs/pat-knowledge-contract.md`** |
