...
| M4 — increment 2 | **NEXT (same owner go)** — the `stock_chart_v3.py` fork (spec §3): dense rail · `?cmp=` chart authority + carryover · benchmark selector · series CSV · seam tests | the hub's chart section currently renders the honest placeholder + classic spoke |
