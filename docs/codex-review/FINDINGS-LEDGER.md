...
- **D1-F1 delivered-value split bug** — `signals.py:_character_arrays` `deliv_value` now uses **raw** same-day close (was raw_qty × adjusted_close). Compiles; Codex confirmation pending (consult-1 Part 1a). ✅ **ACTIVATED ON VPS 2026-07-14** — deployed HEAD `signals.py` (full-scp, md5-verified; box was an older committed version, no uncommitted content; this also brought the undeployed D1-F5 read-suffix current), then ran the backfill. ⚠ **CORRECTION to the prescription:** `--relabel-character` is INSUFFICIENT — it only re-derives the `accum_character` LABEL from already-stored numerics; it does NOT recompute `deliv_value_ratio_1m_6m`/`deliv_updown_ratio_3m` (the numerics D1-F1 changed). The correct tool is **`--backfill-triggers`** (recomputes the character numerics from bhav; the note's parenthetical). Ran it (gated past the 16:30 scan cluster; per-symbol/WAL-cooperative): **3,805 symbols, 5,968,171 rows recomputed, 0 failures**, 2h6m. Verified vs a pre-snapshot: **650 of 2,377 latest-date rows changed** (the corp-action-in-trailing-180d subset), **16 character-LABEL flips** (e.g. ARIHANT None→ACCUMULATION). No hermes-api restart needed. **NOTE:** an eager first launch started in the pre-wolfe gap and was killed after ~3 symbols (relaunched with a hard 16:30 time-gate).
