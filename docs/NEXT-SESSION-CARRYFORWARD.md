...
- **Estate notes:** ~~`claude/prud-gate-orthogonal` + `s150-pat-autolearn`~~ **✅ REMOVED (S164-c, Ramana-directed 2026-07-16):** both worktrees + branches deleted after live clean-tree + merged-into-origin verification (`git worktree remove` + `-d`; the read-only-bit gotcha hit again → attrib-clear scoped to the three leftover dirs, the MAINT-WTPRUNE recipe). Root strays `0.83`/`Sharpe`/`surface` remain unowned (MAINT-WTAUDIT flag stands). ~~3 NEW `.git/worktrees` husks flagged~~ **✅ PRUNED (S164-c cont., Ramana-directed):** by prune-time the candidate list had grown to SIX husks (`wt-cat`·`wt-cfu`·`wt-fable`·`wt-slog`·`wt-union`·`wt-X` — running lanes finished + deleted their scratchpad trees; R-bit debris remained). Full WTPRUNE recipe held: candidates-only attrib-clear → atomic re-diff → `git worktree prune -v` exit 0 → `prune -n` empty; live set intact (a sibling even spawned a NEW worktree mid-prune — untouched, the candidate-scoping worked).
