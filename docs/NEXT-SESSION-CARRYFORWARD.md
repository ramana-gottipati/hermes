...
## ✅ 2026-07-22 — 16BD (improve the R logic): C (hold winners longer) SEALED as K30-HOLD, the 5th union sibling — B-proxy inert, governance-B forward-only — do NOT redo; kickstart-pick-verify
...
- ➡ OPEN follow-ons (kickstart-pick-verify each): A2-HOLD (same lever on A2 = +1.1pp at band 55-60, measured — seal if wanted) · wire K30-HOLD into `union_forward.py` for the Oct-3 mechanical judgment · switch the governance blow-up filter ON as a live forward overlay so its clock starts today.
