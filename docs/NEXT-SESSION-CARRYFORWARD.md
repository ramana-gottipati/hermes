...
- **Estate notes:** `claude/prud-gate-orthogonal` (empty claim worktree+branch at `25a3ded`) is SUPERSEDED by S164 — removable at Ramana's option; `s150-pat-autolearn` branch+worktree are fully merged into origin (`ca7ca93` ∈ origin/main) — same; root strays `0.83`/`Sharpe`/`surface` remain unowned (MAINT-WTAUDIT flag stands).
