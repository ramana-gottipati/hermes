...
- **ORTHOGONAL-DATA EXPLORATION (2026-07-23, all recorded in `docs/strategy-ledger.md`, all on origin):** after the crossover verdict, tested whether NON-price/volume data lifts the book. Results: (a) REGIME overlay = a DRAWDOWN lever on beta books not alpha (trend-200DMA cuts momentum DD −63→−42 flat CAGR; India VIX de-risk LOSES to plain trend); (b) INSTITUTIONAL FLOW (`shareholding_history` DII+FII, on box, PIT) = REAL but weak/unstable — δ+0.07 long-only (R/V 0.51), HURTS as a low-vol tilt (0.60<0.76), long-short spread NEGATIVE (−8% gross, short leg +24%>long +14%; flips sign across windows) → NOT fundable in any form, thread CLOSED. **Fundable product remains standalone low-vol.** Modules `regime_overlay`/`inst_flow`/`inst_flow_ls`/`lowvol_flow_tilt` (descriptive). **NEXT SCOPED EFFORT (not built): options-implied — `docs/options-implied-scope.md`.** OI signals (PCR/max-pain) ALREADY on box (`fno_oi_signals`, 2024-07+, 273 F&O names); IV/skew = the multi-day build; ⚠ ~2yr history caps confidence. Phase 0 (test existing OI signals) is CHEAP + data-on-box — do that first; build IV only if it shows life.
- **Numbers single-sourced:** `docs/strategy-ledger.md` §§ "EMA-crossover family — VERDICT" / "Forward test ARMED (2026-07-23)" / "Study 2026-07-23 — WEEKLY BAND RECLAIM" / Studies 07-22c/22d; PROJECT_STATE **S214** + Key file paths + **D146**; memory `failure-models-ledger`. Commits `c5ac9d0`·`c3903b1`·`d5b1ce7`·`db311fa`·`b4522c8`·PROJECT_STATE `5e1bff7`·weekly `7912bd6`. ⚠ Housekeeping: an unrelated chart/wolfe lane is parked on branch `chart-wolfe-unpushed` (12 commits) — delete once `git cherry origin/main chart-wolfe-unpushed | grep '^+' | grep -icE 'wolfe|chart'` == 0 (was 1 at wrap).
...
- **The spine is CONSUMED + PROVEN:** the sibling-S207 `/dash/seasonal-calendar` lens reads `x_setups_signals.latest(module='calendar_conditioning')` → **X-10's render is DONE**; the spine is validated as consumable.
- **➡ THE OPEN FOLLOW-ON (owner/redesign-gated) = the "Setups" render lens for X-04·X-07·X-09** (the explosive-move SETUP family) — full build spec at **`docs/x-setups-render-spec.md`** (reads `latest()`, prod-venv-safe; the redesign M4 column-architecture decision picks standalone board vs dossier columns; one-session build once the shape is chosen). Coordinate with the redesign lane, do NOT collide.

## ✅ 2026-07-22 — S209 (ESTATE RECONCILIATION + gate-0 fix): the inherited shared checkout had DIVERGED (ahead 8 / behind 20) — the 8 unpushed Wolfe draw-tool commits are now on origin, gate-0 (bare `pytest -q`) restored, one cross-lane note recorded — do NOT redo; kickstart-pick-verify
- **What happened:** boot found `main…origin/main [ahead 8, behind 20]` (invariant violated). The 8 ahead = a finished-but-UNPUSHED Wolfe/chart draw-tool lane; origin had S204–S208. Reconciled by ONE `git merge` (`3fd0acb`; rebase would conflict on `PROJECT_STATE.md` 8×) — union'd the session-log, both lineages verified, pushed. Suite 763/1 (was 759 — wolfe tests merged in).
- **⚠ THE SHARED TREE IS HOT — a sibling absorbed my staged docs TWICE via the shared index** (they `git commit`'d while my files were `git add`ed → their `2184bdb` swept my S209 docs under a wolfe-paint message; content-correct but misattributed). **BINDING LESSON for the next session: work in an ISOLATED worktree (`scripts/new-lane.sh`), NEVER stage into the shared index while a sibling is active.** The S209 wrap itself was completed from an isolated worktree (`git worktree`), rebased onto origin, verified, pushed — origin is now `888a371`. The shared checkout at `D:\patearn` is left DIVERGED at the sibling's local commits (`2184bdb`→`31c6f9f`→`4a90ae5`→`9badff0`, actively worked). **🔴 RECONCILE RECIPE (owner-verified S209, dry-run conflict-free) — whoever reconciles it MUST use `git rebase --onto origin/main 2184bdb` (drops the ghost, replays the 3 real commits), NOT a plain `git pull --rebase`/`git pull`.** `2184bdb` is a GHOST DUPLICATE: its content (the S209 docs + wolfe-paint) is already on origin via the rebased `5b5a190` but is NOT patch-identical (my merge resolution changed its diff), so a naive rebase REPLAYS it and CONFLICTS on `PROJECT_STATE.md` — risking a duplicated S209 entry. The 3 genuine unpushed commits (`31c6f9f` momentum-research · `4a90ae5`/`9badff0` chart-fullscreen) replay cleanly. **Run it only when the shared tree is CLEAN** — at S209 wrap the sibling had uncommitted WIP (`momentum_band_rsi.py`/`wolfe_overlay.py`/`wolfe_view.py` + new `wolfe_learnings_store.py`), so the reconcile was correctly LEFT TO THE SIBLING (owner call, Ramana). Live sibling files seen: `momentum_band_rsi.py` (a pre-registered R-logic study, mid-revision) — untouched.
- **✅ gate-0 FIX (`conftest.py`, `888a371`):** a sibling's `research/explosive_moves/gauntlet/rsirs_denom_test.py` opens a hardcoded `/opt/hermes` path at IMPORT time → crashed bare `pytest -q` COLLECTION off-box (the documented "gate-0 anywhere"). Added `collect_ignore_glob=["research/**/*_test.py"]` — those are box-run scripts, NOT pytest tests (research coverage = the `tests/test_research_selftests.py` wrapper, S197); no coverage lost (bare `pytest -q` == `pytest tests/ -q` == 763). Bare gate-0 is green again.
- **➡ Cross-lane note for the REDESIGN lane (`docs/redesign-coordination.md` §6):** the Wolfe `e6439cc` (fullscreen drawing DRAWER + branded screenshot) drifted `stock_chart.py` md5 `f4608185`→`63148a7f` — the frozen base of the M4 fork `stock_chart_v3.py`. Nothing broke (`BASE_MD5` is a documented constant, not a live assertion); the hub chart just lacks the drawer+screenshot until/unless the owner has the lane regenerate the fork. Owner call, not pre-empted.
- **Queue unchanged (all gated):** ESCALATE bank EMPTY; X-compute stream COMPLETE + X-10 surface LIVE; other X-surfaces owner/redesign-gated (M4 columns); M5–M8 unapproved; union 2026-10-03 time-gated; the now-SEVEN union siblings await Oct-3 `union_forward.py` wiring + family adjudication (owner). No clean non-colliding build-pick — S191's finding holds.

## ✅ 2026-07-22 — S207 (X-10 SURFACE — the render half): the CALENDAR-CONDITIONING LENS is LIVE — `/dash/seasonal-calendar`, a 4th seasonal-family member surfacing expiry/holiday return-conditioning, DESCRIPTIVE-only — built + deployed + walked — do NOT redo; kickstart-pick-verify
- **Built to the SURFACE-PLAYBOOK + LIVE:** `src/web/calendar_conditioning_view.py` reads the bounded `x_setups_signals` snapshot (module='calendar_conditioning' — the S205 spine, NOT the JSON; collision caught pre-commit + rewired) + `x_setups_meta.asof`; sortable sym-linked table (expiry-day/expiry-week/pre/post-holiday return-Δ in bps) + `format=csv` + the 0-certified descriptive fence. Registered (`lens_registry`), mounted (`_ROUTER_SPECS`), 4th `_subnav` chip in BOTH seasonal copies, 4 glossary bullets, Pat `NAV_ONLY`. Gates: full suite **759/1** · Pat 12/12 · route-registry 7/7. Deployed (clean-scp new view + seasonal×2 + glossary; **anchored-insert** `lens_registry`+`v2_surfaces` per §7; `hermes-api` restart 03:45 UTC). **Live walk: 200, REAL data (1926 names, asof 2026-07-21, 60 sym-linked rows, signed-bps deltas), `seasonal-tape` carries the 🗓 chip, CSV 200; nav-integration verified consistent with its sibling `seasonal-tape` (no orphan).** The `hermes-xsetups-scan` nightly timer (sibling `8ec5568`) refreshes the snapshot → no freshness debt.
- **Scope note (avoids a false "X-surfaces are done"):** this is the SEASONAL-family cut of X-10 only — correctly independent of the redesign-coordinated X-setups board. **The OTHER 3 X-surfaces — X-04 overnight · X-07 shelves · X-09 base-breakout (the explosive-move-SETUP family) — remain the redesign lane's SPEC'd/handed-off work (the sibling-S207 "blocked on redesign coordination" note stands for THOSE). Coordinate, do NOT collide.**

## ✅ 2026-07-22 — union_forward.py: 7-sibling WIRING is LIVE + GREEN on the box (ledger 16BG) — era-floor gate re-anchored after the ETF-class CA drift was investigated + a tolerance band added; all 6 rows OK, the 3 siblings reproduce to the decimal — do NOT redo; kickstart-pick-verify
- **DEPLOYED + verified (union_forward.py md5 `7dcb6383`, box == repo):** all 6 gate rows OK; §1b reproduces K30-HOLD 27.2% · A2-HOLD 26.6% · K30-DEEP-HOLD 27.5% (recorded, to the decimal); §3b sibling forward-criteria self-suppress at 0 quarters (fire Oct-3+). The Oct-3 runner now grades all SEVEN siblings mechanically. **The §4 portfolio dial (16AN) now also carries K30-DEEP-HOLD** (the best net + lowest-DD sibling) beside K30/A2 — mixed with the long G-sec ballast at 100/0…70/30 (fires Oct-3+; re-deployed union_forward.py, box == repo).
- **The blocker (RESOLVED, 16BG):** the gate had gone RED because the S185-S189 ETF-class CA backfill ingested 174 ETF splits (ex_date ≤ 2026-04-01) AFTER the 16AW anchors — every one an ETF/index-fund, ZERO equities; the era-floor books select ETFs (16AR) so they drifted, base books EXACT. Legitimate recorded-class repair. FIX: re-anchored A1/A2/K30 (base unchanged) + a 2% era-floor tolerance band (base stays tight = tamper-evidence).
- ➡ WATCH (durable, low priority): the era-floor anchors will keep drifting slightly as the ETF-CA nightly runs — the 2% band absorbs it; a future clean-up could ETF-exclude the era-floor gate entirely if the trickle grows. Base books remain the tight tamper-check.
- **Wiring DONE (`research/explosive_moves/union_forward.py`, committed, NOT yet deployed):** the 3 HOLD/DEEP siblings (K30-HOLD `e6994c19` · A2-HOLD `17e0dd1a` · K30-DEEP-HOLD `b705f770`) wired in ADDITIVELY — engine exec patched with HOLD_BAND + TURN_LO toggles whose DEFAULTS are BYTE-IDENTICAL to the 6 sealed rows (PROVEN: U/B14/C40 reproduce EXACTLY in both the modified and the original runner), a §1b sibling-reproduction block, and a §3b sibling forward-criteria block that feeds the family adjudication. Sealed engine `union_ladder_val.py` UNTOUCHED. Syntax-checked; the SIBS runs completed on the box (no crash).
- **🔴 PRE-EXISTING BLOCKER (NOT the wiring — the DEPLOYED original fails it identically):** the reproduction gate is RED. The ERA-FLOOR books drifted since the 16AS/16AW anchors (2026-07-17): **A1 87.75→88.30 · A2 86.59→86.52 · K30 100.73→100.19** (U/B14/C40 EXACT). Consistent with the documented era-floor data-sensitivity (16AU/16AW class — archive updates move the pf/pf1 floor books). The gate STOPs at A1, halting the whole runner — so §1b/§3b could not be verified end-to-end today.
- **➡ NEXT (before any deploy):** run the 16AS re-anchor loop — (1) **INVESTIGATE** what archive change 2026-07-17→07-22 moved the era-floor books (nightly ingest? corp-action? — the gate demands this before re-anchoring, not a blind update), (2) `union_forward.py --derive-anchors` (current: A1 88.30 · A2 86.52 · K30 100.19), (3) update the `GATE` dict + record a ledger entry, (4) THEN deploy `union_forward.py` (scp + md5) and verify §1b/§3b run green. Box left clean: the `union_forward_test.py` probe was removed; the deployed runner is untouched (md5 == origin).

## ✅ 2026-07-22 — 16BF: the STACKED variant SEALED as K30-DEEP-HOLD (7th union sibling) — deeper-turn<20 × hold-band top-60 combines BOTH gains (net 17.8→19.2% AND drawdown −38→−29%) — do NOT redo; kickstart-pick-verify
- **K30-DEEP-HOLD SEALED (`docs/prereg/union-k30-deephold-prereg.md`, sha256 `b705f770…`):** COMPOSITE-30 + two orthogonal levers (deeper-oversold turn <20 from 16BE + hold-band top-60 from 16BD). Net-of-gauntlet **17.8→19.2%** (₹1cr 27.6→34.8cr), worst-drop **−38→−29%** — the best net AND lowest drawdown of all 7 siblings. turn<25 stack (19.1/−33) confirms robustness. Forward 2026-10-03+ at ≥8 quarters.
- ⚠ **HIGHEST overfit risk of the siblings** (TWO stacked in-sample levers) — disclosed in the prereg; deflate hard for the forward view. Adjudication still picks ≤1 across all 7 (shared levers/base disclosed).
- Runner `research/explosive_moves/gauntlet/build_stacked_gauntlet.py`. Full record: ledger 16BF + memo §12.
- ➡ The union family now has SEVEN sealed siblings (union · β14 · C40RA · K30 · K30-HOLD · A2-HOLD · K30-DEEP-HOLD). OPEN: wire all into `union_forward.py` for the Oct-3 mechanical judgment.

## ✅ 2026-07-22 — 16BE (RSI-of-RS reversal, two forks): SECTOR≫INDEX (sealed choice confirmed) + a DEEPER oversold threshold is a DOUBLE-WIN in the union (+0.9pp net AND −5 to −9pp drawdown) — do NOT redo; kickstart-pick-verify
- **Traced first (the answer to "which RSI?"):** the union's TURN leg runs RSI on **stock ÷ SECTOR** (`union_ladder_val.py:224/237`); the TREND leg runs RSI on the **stock's own price**; qualify on EITHER.
- **Sector vs index (the untested fork, now tested — `rsirs_denom_test.py`):** sector-relative RSI-of-RS reversal GEO +0.47%/q SIG; INDEX-relative (stock/Nifty500) GEO −0.48% (fails the positive-geometric bar), ns. **Keep sector-relative.**
- **Deeper oversold = DOUBLE-WIN in the union (full-book gauntlet, `build_reversal_gauntlet.py`):** TURN floor <30→<25→<20 lifts net AND cuts drawdown, monotone: **17.8%/−38% → 18.7%/−33% → 18.6%/−29%** (K30 config, gate reproduces). Robust across both deeper thresholds.
- **Standalone reversal book NOT viable** (~11% net, below index, thin) — needs the TREND leg; buried.
- ➡ STRONG SEAL CANDIDATE (owner call): the union with **TURN<20** — recommend sealing, ideally STACKED with the K30-HOLD hold-band (16BD) to combine both gains (hold-longer +1.1pp/DD-flat × deeper-turn +0.9pp/DD−9pp). Full record: ledger 16BE + memo §11.

## ✅ 2026-07-22 — 16BD (improve the R logic): C (hold winners longer) SEALED as K30-HOLD (5th sibling `e6994c19…`) + A2-HOLD (6th, lower-DD `17e0dd1a…`) — B-proxy inert, governance-B forward-only — do NOT redo; kickstart-pick-verify
- **C SEALED (K30-HOLD, sha256 `e6994c19…`, `docs/prereg/union-k30-hold-prereg.md`):** hold a name while it stays in the top-60 (2× the 30 held); refill from top-ranked non-held. K30 net-of-gauntlet **17.8→19.0%** (₹1cr 27.6→33.6cr), worst-drop **−38% UNCHANGED**. Robust across bands 40-60 (+0.6 to +1.2pp, DD pinned). COMPOSITE-30 verbatim except the hold-band; same 4 criteria; near-identical to K30 (adjudication picks ≤1 among passers). Forward-judged 2026-10-03+ at ≥8 quarters. IN-SAMPLE — deflate forward.
- **B-proxy (price-crash filter) BURIED — inert** (0 names removed/rebalance; momentum already excludes crashed stocks). **Governance blow-up filter (idea B) FORWARD-ONLY** — pledge/promoter-sell/surveillance/insider feeds have no PIT history pre-Nov-2025; can't backtest, only run live. Intent sound; evidence missing.
- Runners (additive, sealed engine untouched): `research/explosive_moves/gauntlet/build_c_bproxy.py` + `build_band_sweep.py`. Full record: ledger 16BD + `docs/zerodha-cost-gauntlet-2026-07-18.md` §10.
- **A2-HOLD SEALED (16BD, `docs/prereg/union-a2-hold-prereg.md`, sha256 `17e0dd1a…`):** the same 2×-holdings hold-band on A2's equal-weight top-40 base (top-80). Net **17.2→18.6%** (₹1cr 25.0→31.5cr), worst-drop **−33% UNCHANGED**; robust bands 60-90 (+1.1 to +1.6). ⚠ EW-undercharge → mildly optimistic net; its case is the shallower drawdown, not a higher net than K30-HOLD. Runner `build_a2_sweep.py`.
- ➡ OPEN follow-ons (kickstart-pick-verify each): wire K30-HOLD + A2-HOLD into `union_forward.py` for the Oct-3 mechanical judgment · switch the governance blow-up filter ON as a live forward overlay so its clock starts today.

## ✅ 2026-07-22 — S207 (DATA SPINE DEPLOYED): `x_setups_signals` + its 4 X-callees + a nightly timer are LIVE on the box — the 4 X-modules now produce REAL nightly signals; render lens now blocked ONLY on redesign coordination — do NOT redo; kickstart-pick-verify
- Deployed (scp + verify, no restart — not web-imported): 5 new files + `hermes-xsetups-scan.{service,timer}` (17:00 UTC Mon–Fri, runs under `.venv-research`, enabled-not-started per AUD-95). Fixed an as-of bug (survivorship-inclusive scans → pinned to `MAX(EQ trade_date)`, current-trading names only). Latest bhav 2026-07-21; ~4626 current rows (bb 116 · on/vs ~1520 · cal ~1470). **venv bridge = `.venv-research`** (numpy 2.2.6); `latest()` is numpy-free so the web reads it under the prod stdlib-only `.venv`.
- **Render lens = the remaining follow-on:** now UN-blocked on the box (spine live); still owner/redesign-gated (M4 column architecture — X-metrics may land as dossier COLUMNS rather than a standalone board). Reads `latest(conn, module=...)`. Spec in the S205/S207 PROJECT_STATE entries.

## ✅ 2026-07-21 — S205 (surface build, step 1 — DATA SPINE): `src/automation/x_setups_signals.py` — nightly pre-compute materialising the 4 X-scans into `x_setups_signals`; render lens SPEC'd + handed off — do NOT redo; kickstart-pick-verify
- launchpad_signals pattern: isolated table (db.py untouched) · `compute_all`/`persist_scan`/`latest` · `--persist-scan` CLI · hermetic selftest (synthetic round-trip, zero-hit validity, idempotency; no bhavcopy) · `tests/test_x_setups_signals.py`. Full suite 738 → 739. Takes the 4 X-modules from hermetic-selftest to REAL nightly signals. Additive, no product source, no deploy.
- **🔜 RENDER LENS = the OPEN follow-on (owner/redesign/box-gated — do NOT force a half-page; SURFACE-PLAYBOOK forbids it):** a Strategies "Setups" lens reading `latest()` (cross-link Launchpad = pre-breakout ↔ X-09 = breakout), full checklist (education + glossary for x09_score/on_share/base_length + Pat DATA+NAV + fence + CSV + sym links + home=opt-in). **Blocked on:** (a) box deploy of `x_setups_signals` + a nightly `hermes-xsetups-scan.timer`; (b) the redesign lane's M4 column architecture (X-metrics may land as dossier COLUMNS — an owner call). Full spec in the S205 PROJECT_STATE entry.

## ✅ 2026-07-21 — S204 (charter X-10, compute layer): expiry/holiday conditioning UTILITY SHIPPED — `research/explosive_moves/calendar_conditioning.py`, hermetic selftest under CI (737 → 738); the X-compute stream is COMPLETE — do NOT redo; kickstart-pick-verify
- Precise trading-calendar utility (primary-source, calendar-only): `monthly_expiry_dates` (last trading day ≤ last Thursday; holiday rolls to prior TD) · `calendar_flags` (is_expiry · days_to_expiry · expiry_week · pre/post_holiday from calendar-gap detection · prior_gap) · `condition_series` (DESCRIPTIVE per-flag return stats). **FENCED:** the expiry STUDY is owned by the frozen `seasonal_tape` estate (F-EXPIRY + F-TOM-SIP, largely 0-certified) — this is the missing precise-expiry + holiday-adjacency PRIMITIVE it lacks, NOT a new edge; the read is a conditioning lens against that null prior. Reuses `common.load_series`; `scan()` box-runnable. Additive new module, no product source, no deploy.
- **X-compute stream COMPLETE:** X-04 · X-07 · X-09 · X-10 all shipped + CI-covered (harness 20). Remaining X = box-gated (X-06 Amihud) / reframed-only (X-08). **NEXT beyond compute = the SURFACE half of each X-item** (SURFACE-PLAYBOOK builds: new lens + Pat/glossary/education + box deploy) or owner/time-gated items.

## ✅ 2026-07-20 — S202: SEAL-INTEGRITY MONITORING is now AUTONOMOUS (break self-pages · heartbeat confirms) — `c5256f0` deployed + box-proven; the 15h session-watcher approach is RETIRED — do NOT redo; kickstart-pick-verify
- **The autonomy loop (no agent/session in it):** a not-ok nightly `chk_prereg_seals` DMs Ramana via `alert-telegram.sh --text` (`data_quality.py::_page_on_seal_break`, CLI-only so `run()` stays pure), AND the daily 03:30 heartbeat carries `seals ok | BROKEN <date> | stale <date> (Nd)` (`estate_heartbeat.py::_seal_status`) — the stale-check makes **silence≠success**. Bespoke page because the seal is a deliberate WARN (never fails the service → the live `OnFailure=hermes-alert@` can't fire on it); severity NOT flipped, no kill-switch coupling.
- **Box-proven (real data):** test page reached Telegram (`paged 282907906`); a clean `--run` fired NO page; live heartbeat reads `… crit 2 · seals ok · inbox clear …` GREEN. 3 files md5==HEAD (`.bak-s202`), no systemd/schema change; both timers armed (heartbeat 03:30 · data-quality 06:31 UTC). Full suite **736 passed / 1 skip**. Watcher `b2sl4xs5g` dropped (it died on session teardown — proof a live session can't be the watcher; TIL in the S202 PROJECT_STATE entry). Also fixed: `os.uname()` is Unix-only → `socket.gethostname()`.
- **Follow-on — SELF-VERIFYING PAGER (`e0993a1`, deployed):** `scripts/alert-telegram.sh` now parses Telegram's `ok:true` and logs the `message_id` (or the API rejection reason on failure) instead of fire-and-forget — `curl` exit 0 was NOT proof of delivery (no `-f` → HTTP 200 `{"ok":false}` on a bad token/blocked chat still printed "paged"). Closed the gap that made the S202 test DM unconfirmable; delivery is now **server-confirmed (message_id 410)**. Backward-compatible (`%n` + `--text` modes); still ALWAYS exits 0 (AUD-26 no-cascade); benefits all ~18 units that page via `hermes-alert@`.
- **Nothing owed for this lane.** Standing queue unchanged: charter X-compute stream (X-09 base-length×breakout-velocity · X-10 expiry/holiday) · redesign M4–M8 (owner-gated) · union **2026-10-03** forward test (time-gated).

## ✅ 2026-07-21 — S203 (charter X-09, compute layer): base length × breakout velocity SHIPPED — `research/explosive_moves/base_breakout.py`, hermetic selftest under CI (736 → 737) — do NOT redo; kickstart-pick-verify
- O'Neil base/breakout made quantitative + PIT: `base_length` (prior-peak→breakout days) × `breakout_velocity` (realized %-above-pivot/day since breakout, never past as-of) = `x09_score`; a failed breakout (back below pivot) scores NEGATIVE; + base_depth, vol_surge. ADJUSTED prices (split can't fake a breakout); fresh-crossing detection; short-base (<10d) pops filtered. Reuses `common.load_series`/`eq_symbols`; `scan()` box-runnable. Additive new module, no product source, no deploy. Distinct from launchpad COILED (boolean state) + ignition (delivery intensity).
- **Follow-on (open, surface half):** breakout board / columns = a SURFACE-PLAYBOOK build. **X-compute stream: X-04 · X-07 · X-09 done; remaining pure-compute = X-10 expiry/holiday conditioning** (X-06 Amihud migration-delta is box-gated).
## ✅ 2026-07-18 — S201 (charter X-07, compute layer): volume-at-price shelves SHIPPED — `research/explosive_moves/volume_shelves.py` (POC / value area / shelves), hermetic selftest under CI (735 → 736) — do NOT redo; kickstart-pick-verify
- Daily-bar volume profile: each day's ₹ turnover spread uniformly over `[adj_low, adj_high]` → high-turnover bins = shelves, modal bin = POC, narrowest 70%-turnover band = value area; labels last-close vs VA. ADJUSTED prices (split can't smear the profile); turnover conserved. Reuses `common.load_series`/`eq_symbols`; `scan()` box-runnable. Additive new module, no product source, no deploy. Reused the algorithm PATTERN of `rsband._poc_value_area` (count-based RS sibling), not its code.
- **Follow-on (open, the surface half):** shelf overlay / board columns = a SURFACE-PLAYBOOK build (lens + Pat/glossary/education + box deploy); run `scan()` on the box for the first real-data read. **The charter X-compute stream is a repeatable non-colliding lane:** X-04 (S199) + X-07 (S200) done; remaining pure-compute picks = X-09 base-length × breakout velocity · X-10 expiry/holiday conditioning (X-06 Amihud migration-delta is box-gated).

## ✅ 2026-07-18 — S199 (charter X-04, compute layer): overnight vs intraday return split + overnight-pump flag SHIPPED — `research/explosive_moves/overnight_split.py`, hermetic selftest under CI (734 → 735) — do NOT redo; kickstart-pick-verify
- Exact LOG split (`on = ln open/prev_close`, `intra = ln close/open`, `on+intra = total`), CA-safe (NSE `prev_close` split-clean + `is_ca` mask). DESCRIPTIVE overnight-pump flag = a ≥ +10.5% window move that's ≥ 90% overnight. Reuses `common.load_series`/`eq_symbols`; `scan()` is box-runnable. Additive new module, no product source, no deploy.
- **Follow-on (open, the "columns" half):** surface X-04 as board columns + a pump-flag badge — a SURFACE-PLAYBOOK build (new lens + Pat/glossary/education + box deploy). Analytical core is done; run `scan()` on the box for the first real-data read, then wire the surface.

## ✅ 2026-07-18 — S197 (verified-open pick): the pre-registered event-study battery is under CI for the FIRST time — `tests/test_research_selftests.py` wires 16 orphaned `research/explosive_moves` selftests into pytest (718 → 734 passed) — do NOT redo; kickstart-pick-verify
- The E-02/03/04/11/12/14 event studies + PEAD/footprint + rating/dividend/insider/rebrand drift + SHP/reclaim/fractal selftests ran only via a manual box `--selftest` (zero pytest coverage → silent-break risk). Now a parametrized wrapper (blessed `test_band_lock` idiom) owns all 16 verified pure-compute + deterministic + no-`research.db` modules. Additive test-only, no product source, no deploy.
- **Excluded (deliberate):** `prereg`/`exit_lab` (S194 prereg-seal lane), `streamband*` (union lane), `rule_lab_executor`/`embase`/`factory` (already covered), and **`fractal_fences`** — a real finding: NOT importable as a package (`research.explosive_moves.fractal_fences`) because it pulls `cost_participation`, part of the bare `from explosive_moves.X import` family that only runs with `research/` on `sys.path`. Documented in the test docstring; not fixed (box-run convention, union-lane turf).

## ✅ 2026-07-18 — S196 (Ramana real-money question): THE ZERODHA REAL-COST GAUNTLET — factor baskets DON'T beat the index net of cost, the UNION family DOES (K30 +17.8% / A2 +17.2% vs index +11.7% in-sample) — full record `docs/zerodha-cost-gauntlet-2026-07-18.md` (ledger 16BC); do NOT redo; kickstart-pick-verify
- Per-name gauntlet (tier spread + 0.5×ATR slip + real Zerodha charges ~0.2%/yr + 20% STCG); validated to reproduce `cost_realism.csv` + every sealed union CAGR to the decimal. **Codex validated both runs.** Key mechanism: cost = market-impact × turnover, not brokerage.
- Factor baskets: best +11% (STEADY LowVol-Mom, ₹8.89cr), below index +12.5% (₹12.72cr); fast momentum go negative. Union: **K30 +17.8% (₹1cr→₹27.77cr), A2 +17.2%** — survive because quarterly + liquid + stops (cost 7.7%/yr vs fast baskets 20-40%).
- K30: survives T+1 execution-lag (+16.7%); AUM ladder capacity ceiling **~₹25-50cr (personal-scale)**. Deliverable = 17-sheet xlsx on Ramana's Downloads/Desktop; scripts `research/explosive_moves/gauntlet/`.
- Caveats: IN-SAMPLE (net-of-cost, not net-of-selection); EW union books slightly optimistic (K30 clean); AUM-blind. **⏳ The ONE thing owed: the sealed 2026-10-03 forward test** — the real OOS judge; everything else (in-sample beat, cost survival, execution lag, capacity) is measured + Codex-checked.
- Bonus (original ask): live bullish screen 2026-07-17 — Next 50 = the narrow index; leadership = Power/Electricals/Capex/Defence + Pharma/Healthcare.

## ✅ 2026-07-18 — S194 + S194b + S194c: the ID-UNIQUENESS GATES are LIVE (doc_hygiene_gate Check D = ledger tags, Check E = Decision-log D-numbers) — the S187 flag's durable L4 fix + its higher-blast-radius twin; two real pre-existing collisions surfaced and BOTH now HEALED (`15i` → `16BB` renumber, S195 — full ref-sweep + strategy-ref sanitizer fix DEPLOYED · `D68` → D145 renumber, S194c) — do NOT redo; kickstart-pick-verify
...
- **D143 framework** (`docs/strategy-families-framework.md`, Decision-log D143): CAGR>15% = a SCREEN not a verdict; register by family + status; **only LOWVOL_MOM/STEADY-25 is fundable net-of-cost.** Phase 4 SHIPPED LIVE on `/dash/model-portfolios` (4 tiers): Tier-D relabelled not-fundable · STEADY-25 = Fundable Core (net **18.1% @₹50cr → 16.5% @₹100cr, ceiling ~₹150cr**) · Tier-B ballast overlay (`16AZ`) · Tier-C graduating candidate (COMPOSITE-30).
- **Drawdown-lever experiment `16BA`** (`/tmp/dd_levers.py`): net-of-cost, controlled — LOWVOL_MOM keeps the top net return (18.0%) at the shallowest DD (−21.8%); the low-vol blend is THE drawdown lever. **Factor baskets** `853d5ae`: MOM6 + pure-LOWVOL baskets built in `factor_league` + live on `/dash/factor-league`.
- **Ledger-tag note (RESOLVED):** the S187-flagged `16AX`/`16AY` collisions were renumbered by the S189/S190 lane — my entries are now **16AZ (ballast) · 16BA (dd-levers)**; all citations reconciled. The S187 FLAG below is CLOSED.
- **➡ NEXT for this lane (nothing forced; kickstart-pick-verify EACH):** (a) the union-universe ETF-exclusion task's owner decision (it will bring sealed-spec options — changes sealed numbers + the 2026-10-03 forward test) · (b) D143 §7 governance tail (ratify promote/retire §6; value/quality/residual baskets need the fundamentals join) · (c) portfolio-layer optional: rebalance-band measurement, GOLDBEES feed-protocol formalisation · (d) time-gated: the **2026-10-03 union forward test**.
...
- **Remainder picks — kickstart-pick-verified S191(cont.), CORRECTED (my first-pass list was over-optimistic):** ~~Wolfe §C runnable-PIT backtest~~ **DONE** (phase1 tradesim + phase2 true-OOS + phase3 beta-control + the 2026-07-11 D111 re-validation; run-book §0a) — NOT open · Wolfe D95 tape-wiring / fractal-visibility = **genuinely open but OWNER-CLAIMED** (the S89 lane owns `wolfe.py`/`wolfe_view.py`/`wolfe_overlay.py` — coordinate, don't collide) · X-04 overnight-split+pump-flag and the SSE live-tape face = genuinely open but **SUBSTANTIAL SURFACE-PLAYBOOK builds** (new lens + Pat coverage + education scaffold + box data/deploy), not quick picks. Net: no clean self-contained quick-pick exists right now — a forced build would collide with owned/gated surfaces, so S191 correctly stopped at verification rather than manufacture risk.
- **Wrap verification (S191 cont.):** secrets hygiene CLEAN (`.env` untracked + gitignored, only `.env.example` tracked; zero live `sk-ant`/`AIza`/telegram-token patterns in tracked non-doc files) · full gate suite **715 passed / 1 skip** green · estate integrity gates (retvol-label, doc-hygiene, DOC_INDEX) all pass.

## ✅ RESOLVED (S193) — ~~FLAG (S187, cross-lane): the LEDGER holds TWO live tag collisions — `16AX` ×2 and `16AY` ×2~~ — the S189/S190 lane RENUMBERED the D143 entries (STEADY-ballast 16AX→**16AZ**, dd-levers 16AY→**16BA**); ledger tags are unique again and all citations (framework §8, live box, factor baskets) point to 16AZ/16BA. do NOT re-chase
...
- **✅ DATA-QUALITY BLOCKER — RESOLVED S182 (ledger `16AQ`; `task_405db4df` closed):** the audit found it was **14 gold-ETF subdivisions across 13 ETFs** (not just GOLDBEES) missing from `corporate_actions` — root cause: the NSE `index=equities` CA feed omits the ETF instrument class. All back-filled via `scripts/backfill_etf_splits.py` (idempotent, primary-source NSE-bhavcopy-derived); GOLDBEES research-adjust now prints native **+11.9%** (was −16.9%), verified by content on the box. ⚠ Separate/disclosed: the production `src/automation/adjust.py` path still can't render 100:1/50:1 subdivisions (no consumer wires the tape; `0.02<v<50` guard) — live impact ~nil (equity universe, not ETFs).
- **➡ NEXT for the gold leg (in order):** ~~(1) the DQ fix~~ **✅ done (S182)** → (2) formalise GOLDBEES via the feed protocol (manifest + DQ + pull-on-demand) → (3) the proper **three-asset book/G-sec/gold** grid (legs mutually independent at −0.12). The §6 rebalance-band measurement (design-doc §10.1) is runnable but LOW-value (G-sec-leg turnover only) — deprioritize.
...
- **➡ QUEUED from §7.7 B (additive, kickstart-pick-verify EACH):** ~~(a) positive CI assertion (no `UNCLASSIFIED_FEEDS` fetcher imported by `src/web`)~~ **✅ DONE `53e7762`** (AST import gate + teeth + bite-proven) · ~~(b) per-feed retirement in `feed_manifest.py` + resolve the `TODO(Ramana)`~~ **✅ DONE** (RETIRE: condition on all 6 + both TODOs resolved + a ratchet test) · ~~(c) primary XBRL/BSE replacements into `FEEDS` with DQ~~ **✅ DONE (this commit)** (all 3 already Feed rows; added the missing `shareholding_xbrl` freshness DQ check — closing `chk_feed_freshness`'s documented residual via the XBRL broadcast date) · ~~(d) `auto_analyst.py` comment PROPOSAL→RATIFIED~~ **✅ DONE**. **All four §7.7 Option-B guards SHIPPED — §7.7 fully executed.** Standing non-§7: the rf re-cut (S170 plan — run when the union lane quiets) + time-gated Aug-1 churn / Oct-3 union forward test.
