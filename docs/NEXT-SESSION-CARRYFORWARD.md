...
- **🔴 DATA-QUALITY BLOCKER (found + spawned as task `task_405db4df`):** GOLDBEES's **100:1 split (2019-12-19, ratio 0.01) is NOT in `corporate_actions`** → `adjust.py` silently corrupts it (uncorrected native CAGR −16.9%/vol 29.5% — one fake −99% quarter; caught by a gold-INR sanity floor). Manual back-adjust (×0.01 pre-split) → sane +11.9%/14.1%. **The proper fix (add to `corporate_actions` + audit peer gold ETFs KOTAKGOLD/AXISGOLD/…) affects ANY consumer of adjusted GOLDBEES prices — it is the gate for the gold three-asset grid.**
- **➡ NEXT for the gold leg (in order):** (1) the DQ fix (spawned task; kickstart-pick-verify — confirm split still missing) → (2) formalise GOLDBEES via the feed protocol (manifest + DQ + pull-on-demand) → (3) the proper **three-asset book/G-sec/gold** grid (legs mutually independent at −0.12). The §6 rebalance-band measurement (design-doc §10.1) is runnable but LOW-value (G-sec-leg turnover only) — deprioritize.
