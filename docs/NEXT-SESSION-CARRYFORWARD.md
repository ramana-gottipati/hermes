...
- Suite 685/1-skip (telegram module, expected). Deploy in progress this same session — `dashboard.py` is D80-forked (patch/anchored-insert, never full-file scp); `review_inbox_view.py` base-matched. Live-verify via curl (anonymous → no banner trace; owner header → the real line) before calling it done.
...
- **▶ RESIDUAL (unchanged, the highest-value next pick):** banks are PROVISIONAL — HDFCBANK scores T4 on RoE alone because its GNPA/CET1/RoA aren't in the archive yet. The **S154 bank re-ingest** (clear bank urls from `fundamentals_xbrl_seen`, or Phase-3 backfill; gate past 14:01/16:00 clusters) is what makes Doctrine-D's data real. NBFCs already have GNPA (Screener-era). Also open: SA-sourced prudential rows stamped SOURCE_CONSO (per-metric override in `write_rows`).
