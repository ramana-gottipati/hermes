...
## ✅ 2026-07-18 — S194 + S194b: the ID-UNIQUENESS GATES are LIVE (doc_hygiene_gate Check D = ledger tags, Check E = Decision-log D-numbers) — the S187 flag's durable L4 fix + its higher-blast-radius twin; two real pre-existing collisions (`15i`, `D68`) surfaced + grandfathered — do NOT redo; kickstart-pick-verify
...
- **Check E's finding — `D68` is a LIVE collision:** two distinct decisions share it (GLM-5.2 code reviewer, session 39 · rotation-vocab/mini_rrg, session 38), both cited across docs AND code; already acknowledged in `docs/strategic-review-2026-07-07.md` ("D68/D79-D80 numbering collisions") but never gated → now grandfathered + gated. Suffixed variants (`D142-RECONCILE`, `D33-web`, `D49g`) are correctly distinct.
