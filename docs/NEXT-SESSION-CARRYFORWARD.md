...
## 🔒 2026-07-15 CLAIM (frosty-darwin lane, S158) — THE REVIEW-INBOX LENS + the Q1 legacy-writer endgame
- **Verified unclaimed + non-existent before claiming:** zero `review_inbox`/`inbox_adapters` references in `src/web`+`src/pat`; no inbox lens in `lens_registry`; only the rulelab claim below is live. **Why this is the pick:** S157 wired the judgment loop but its ONLY human interface is `ssh … review_inbox --decide` — 3 tag proposals + 2 auto-analyst briefs sit pending, 295 decisions sit in the corpus, and plan §2's L5 mandate ("nothing counts until checked, verified and evaluated with human input") is structurally unmet without a surface. It also blocks **L6**: LANE-E's ₹-metered briefs can never be approved → never publish. The lens is the bottleneck of the whole L5/L6 stack.
- **Scope:** NEW `src/web/review_inbox_view.py` → `/dash/inbox` per `docs/SURFACE-PLAYBOOK.md` (full checklist, same session) + **Q1's endgame** — the legacy `/dash/tags-review` writer gets BRIDGED through the inbox (additive-never-replace: Ramana keeps his bulk ergonomics, the corpus stops being bypassed), NOT deleted. Bridge helper lands in `inbox_adapters.py` (adapter pattern); forked `cockpit`/`dashboard`/`lens_registry`/`v2_surfaces` get anchored inserts only (S-rotation-e precedent).
- **Siblings: `/dash/inbox` + `review_inbox_view.py` + the tags-review bridge are MINE this session.** rulelab stays cranky-khayyam's; the brief wire-publisher is deliberately left for its own session (it needs an approval to exist first).

## 🔒 2026-07-15 CLAIM (cranky-khayyam lane = the dispatched RULE-LAB session) — INTEGRATING `rulelab-d134` @ `b67509d` (LANE-H build)
- **kickstart-pick-verify found the LANE-H build already COMMITTED on the sibling's local branch** (`rulelab-d134`, worktree of session `69963ef7…`, committed 14:26 IST, unpushed; 7 files / +2,068 — `rule_lab.py` engine · `rule_lab_executor.py` (research side) · `rule_lab_inbox.py` producer · `rulelab_flow.py` Pat flow · `rule_lab_view.py` · 2 test files; **zero shared-file edits**, per build-lane bans). This lane was dispatched as the rule-lab BUILD session, arrived second while the build was mid-flight, and yields the build per multi-session-safety — it takes the **INTEGRATION half** instead.
- **Scope of THIS claim:** design-conformance review vs `docs/rule-lab-design.md` (§2 closed-vocab 1:1 binding · §4 verdict invariants · §5.1 trigger map · §6 SEBI fences) → cherry-pick `b67509d` (authorship preserved) → the shared-file half of the §7 playbook (lens row + v2 mount + Pat wiring + pat-coverage/education/route gate tables + glossary keys + `docs/strategies/rule-lab.md` + design-doc retirement per its banner) → gates + full suite → push FF → deploy per deploy-reality (anchored inserts on box-forked `lens_registry`/`v2_surfaces`; writer-safe restart; NO unit starts) → §1 ledger H + plan §4-H + PROJECT_STATE + this baton.
- **Builder sibling (`69963ef7…`): do NOT self-integrate/re-wrap `b67509d`** — it lands via this claim; if we race, `git cherry` shows patch-equivalence — drop yours on rebase. **frosty-darwin: rule-lab is covered here; inboxwire remains entirely yours.**
...
- **⚠ Suite debt (research lanes', not yours):** `research/explosive_moves/combo_test.py` + `tests/test_embase_deliv_value.py` fail COLLECTION without numpy (no `importorskip`) — reproduced at bare `1ca99a5`.
