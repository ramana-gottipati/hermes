...
## ✅ 2026-07-18 — S194: the LEDGER TAG-UNIQUENESS GATE is LIVE (doc_hygiene_gate Check D) — the S187 flag's durable L4 fix, which the renumber (S189/S190) and S192 both LEFT UNBUILT; a real pre-existing `15i` collision surfaced + grandfathered — do NOT redo; kickstart-pick-verify
- **What landed:** `scripts/doc_hygiene_gate.py` **Check D** + `tests/test_doc_hygiene.py::test_ledger_tags_are_unique` — a 4th ratchet check that extracts the dated tag (`2026-07-16AX`) from every `### 2026-…` strategy-ledger heading and fails any duplicate not on the grandfather floor. Blocks the tag-race (16AO/16AX/16AY class) at commit time. Bite-proven (injected `16BA` dup → gate exit 1 + pytest fail → reverted); full suite **717 passed / 1 skip**. Dev-time gate, no deploy.
...
- **⚠ Documented debt (grandfathered, NOT healed):** `2026-07-15i` is a REAL pre-existing collision — two distinct entries (the PIT-sector DATA-AUDIT blocker cited `§15i` by `codex-stock-selection-brief.md`; the SIGNIFICANCE PASS that drove **D139** cited `§15i`/`15i-sig` by this file). Citations disambiguate by context; renumbering a historical heading would rewrite other sessions' genuinely-ambiguous refs (§4.5), so it sits on `GRANDFATHERED_DUP_TAGS`. A background chip was spawned for the eventual clean renumber. To heal later: renumber the DATA-AUDIT `15i` → next free tag, fix the 3 codex-brief refs, drop it from the floor (SEED mode reprints the floor).
