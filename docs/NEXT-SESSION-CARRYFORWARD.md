...
- **S123-b (same session): acknowledge/dismiss — the rail is now a TRIAGE INBOX + LIVE (`ea7451c`, also unpushed).** `acknowledged_at` col (idempotent ALTER migration) + `GET /dash/attention/ack?id=N|all=1` (the one web-write in this face, 303 back, defensive) + per-row `✕` + "dismiss all" (LIVE-view only, not on a `?as_of` replay). Deployed + live ack round-trip verified (30→29→restored). **Full bus suite 54 pass.** Disjoint from the codex + seasonal lanes (bus-owned files only, no forked file). **The LAST unbuilt bus face is now the SSE live stream** (low value at the nightly cadence — nothing streams intraday; consider the Telegram-push follow-on instead). Full record: PROJECT_STATE § Session 123 (+ S123-b).
