...
- **⚠ Observations for owning lanes (NOT mine):** (1) `install-systemd.sh --check` shows **`hermes-deals.timer` DRIFT** (repo≠/etc) — whoever changed it owes the install decision; (2) the 4 UNCAPTURED seasonal units remain (S138 flag); (3) local D:\Hermes main FF'd this wrap if clean — the `stock_chart.py`/`symbol_search.py` working-tree mods there are the Pat lane's own content, since committed as `b7553a6`.
