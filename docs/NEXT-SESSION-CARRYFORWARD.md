...
## 🔒 2026-07-15 CLAIM (cranky-khayyam lane, S159) — THE WIRE-PUBLISHER for APPROVED briefs (L6's last mile)
- **Verified open before claiming:** no publisher module exists anywhere (`git ls-files | grep -i publish` = 0); `auto_analyst.py:6` still says *"publishing APPROVED briefs to the wire is a separate later step"*; it is the **#1 pick on BOTH the S157 and S158 batons** and no claim was live. S158 unblocked it by making approval possible at all.
- **🔴 THE DESTINATION IS NOT THE WIRE — a real finding that reshapes the pick:** `/dash/wire` renders `sent_news`, whose feed (`news_feed`) is an **UNCLASSIFIED vendor-ToS** source — deliberately held OUT of `feed_manifest.FEEDS` pending Ramana's plan §7.7 decision, and *pinned by a test*. Writing our AI briefs into that table would fuse house content with a vendor-ToS-caveated source (provenance + licence-class confusion, Guardrail #8). **Briefs publish to OUR OWN `published_briefs` table**, rendered on the board the brief already cites (`BOARD_URL = /dash/results-reactions`), never into the news feed. The plan's "wire/dossier" wording gets corrected in §4-E with this reason.
- **Scope:** NEW `src/automation/brief_publisher.py` — `publish_approved()` (kind='brief' + status='approved' → `published_briefs`; **exactly-once via the existing kind-generic `inbox_apply_log`** — its docstring names item_id as "unique across kinds", the documented adapter extension point; commit-per-item, S153 lesson) + `unpublish()` (a retraction path is mandatory for public AI text) + CLI. Render = an AI-labeled band on `/dash/results-reactions` (fork-check first). **Stock-dossier placement DEFERRED** (forked `dashboard.py`; the board is where the brief already points).
- **Honesty bound (S158's stance, kept):** 0 briefs are approved, and a machine must not fabricate a human verdict → live E2E ends at "published nothing, correctly". Hermetic temp-DB proof + an on-box dry run are the evidence; the first real publish is Ramana's approval on `/dash/inbox`.
- **Siblings:** `brief_publisher.py` + the results-reactions band are MINE this session. rule-lab (S157-b) is landed+live; `/dash/inbox` is S158's.
