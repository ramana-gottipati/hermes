...
## 🔔 2026-07-14 — S138: signal-bus OWNER-DM PAGER BUILT + PUSHED (`41b6a34`), ships DORMANT — deploy+arm is the next actionable
...
- **⚠ Ships DORMANT + NOT deployed.** The nightly auto-push is piggybacked on `signal_events.py --detect` but **gated by `HERMES_ALERT_DM=1`** — deploying sends nothing until that env flag is set on the box. **NEXT ACTIONABLE (a controlled deploy, quiet non-mid-day window per AUD-95):** fork-check `signal_events.py` (md5 vs HEAD-base) → clean-scp the new module + the anchored `signal_events.py` block → remote import-test → writer-safe `hermes-api` restart → `python -m src.automation.signal_alert_telegram --push --dry-run` on the box to preview → then add `HERMES_ALERT_DM=1` to `/opt/hermes/.env` to ARM. It is a PRIVATE owner DM (like season-digest), **NOT** the public approval-gated channel (that stays S-F).
...
- **Next UX picks per the audit §8:** ~~S-H route gate~~ **✅ S133** → ~~S-C items 1+7~~ **✅ S134** → ~~S-C item 2 scaffold~~ **✅ MOSTLY S136 (11/14 pages)** → **NEXT: S-C item 3 (glossary wiring on the "neither" pages) + item-2 remainder (screen2/dashboard-forked/wolfe_trades fold) + item 5 (nav subtitles)** → S-D search/entry. `docs/ux-journey-audit-2026-07-13.md` §8 has the paste-ready statements.
