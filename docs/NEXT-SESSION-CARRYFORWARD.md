...
- **FOLLOW-ON (small): deploy `reversal-context.md` + the reversal-context `_PAGES` line to the VPS** so `/dash/strategy-ref?p=reversal-context` serves live (Origin labels need NO deploy — `_public` strips them). Reconciled in an isolated worktree; explicit-paths only (S-B1 rail + XBRL lanes hot — none absorbed).
