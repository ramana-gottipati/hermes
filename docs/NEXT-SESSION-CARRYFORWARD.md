...
- **Numbers single-sourced:** `docs/strategy-ledger.md` §§ "EMA-crossover family — VERDICT" / "Forward test ARMED (2026-07-23)" / Studies 07-22c/22d; PROJECT_STATE **S214** + Key file paths + **D146**; memory `failure-models-ledger`. Commits `c5ac9d0`·`c3903b1`·`d5b1ce7`·`db311fa`·`b4522c8`·PROJECT_STATE `5e1bff7`. ⚠ Housekeeping: an unrelated chart/wolfe lane is parked on branch `chart-wolfe-unpushed` (12 commits) — delete once `git cherry origin/main chart-wolfe-unpushed | grep '^+' | grep -icE 'wolfe|chart'` == 0 (was 1 at wrap).
