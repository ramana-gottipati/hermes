...
- **✅ Reconciled (D114, S113) — code↔methodology mapping now explicit.** [patterns.md](../../resources/patearn/patterns.md) and `scoring.py` **agree exactly on the top-5 Quality Gate**; their **patterns 6–14 are a deliberate reorganization to what is computable from fundamentals**, so the numbers/labels don't line up 1:1 (code#6 Promoter = methodology#8; code#8 Institutional Neglect = methodology#10; code#9/#10 Earnings Momentum/Margin Expansion are computable adjuncts; methodology#7/#13/#14 are Phase-4-only). The authoritative map now lives in [patterns.md](../../resources/patearn/patterns.md) § "Implementation mapping (scoring.py)", and `scoring.py`'s stale `# Pattern weights from patterns.md` comment is corrected with per-line methodology annotations. **Documentation only — scoring output is byte-identical** (Ramana's call: document the mapping, don't change behavior). Actually re-numbering/re-weighting the live scorer to the canonical methodology is deliberately deferred — it moves every NS + touches `pattern_scores.detail_json` + the surfaces + AUD-15, so it stays a separate signed decision.
